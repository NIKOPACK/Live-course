...
function sanitizeNonTaskEngineOutline(outline: SceneOutline): SceneOutline {
  ...
  // procedural-skill is gated behind taskEngineMode to protect ordinary LiveCourse generation.
  return {
    ...outline,
    type: 'interactive',
    widgetType: 'diagram',
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const {
      ...
      researchContext,
      agents,
      teacherContext: learnerTeacherContext,
    } = body as {
    ...
    if (
      learnerTeacherContext !== undefined &&
      (typeof learnerTeacherContext !== 'string' ||
        learnerTeacherContext.length > MAX_TOTAL_CONTEXT_CHARS)
    ) {
      return apiError(
        'INVALID_REQUEST',
        400,
    ...
    // Build teacher context from the fixed front-facing teacher persona and
    // the bounded learner-only context supplied by the new-course boundary.
    // This route never persists the context; it is prompt input only.
    const teacherContext = [formatTeacherPersonaForPrompt(agents), learnerTeacherContext?.trim()]
      .filter(Boolean)
      .join('\n\n');

    // Homepage product switches are gone (docs/spec/05 S1). Vocational task
    // engine remains a server env gate; leftover interactiveMode must not
    // pick a second outline prompt.
    ...
    const prompts = buildPrompt(promptId, {
      ...
      clarificationContext: formatClarificationForPrompt(requirements.clarificationAnswers),
      selectedTopicsText: formatSelectedTopicsForPrompt(requirements.selectedTopics),
    });
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          if (parsedOutlines.length > 0) {
            // A3 审校：学习者勾选过范围时，检查覆盖/顺序并修复一次（失败放行原大纲）
            if (requirements.selectedTopics?.length) {
              const reviewAICall = async (system: string, user: string) =>
                (
                  await callLLM(
              ...
              const review = await reviewOutlines(
                {
                  requirement: requirements.requirement,
              ...
              if (!review.pass) {
                log.warn(
                  `Outline review failed with ${review.issues.length} issue(s), repairing once`,
                );
                parsedOutlines = await repairOutlines(
                  {
                    requirement: requirements.requirement,
                    selectedTopics: requirements.selectedTopics,
                    outlines: parsedOutlines,
