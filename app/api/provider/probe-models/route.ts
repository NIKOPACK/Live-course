 ...
 *
 * When `providerId` identifies a server-managed provider, the server's own
 * apiKey/baseUrl are used (client never needs to enter or see the key) —
 * same managed/unmanaged precedence as every other provider resolution.
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const baseUrl = providerId ? resolveBaseUrl(providerId, clientBaseUrl) : clientBaseUrl;
    const apiKey = providerId ? resolveApiKey(providerId, clientApiKey) : clientApiKey;
