# ComfyUI Workflow Setup for LiveCourse
...
|------------|-----------------|---------|
| `Input Prompt` | `PrimitiveStringMultiline` | The image description from LiveCourse is injected here |
...
The default Base URL `http://localhost:8188` assumes **LiveCourse and ComfyUI run
on the same host** (the typical local / self-hosted setup).

When LiveCourse runs with `NODE_ENV=production`, a **client-supplied** Base URL
...
- **Same-host / self-hosted:** works out of the box. Server-resolved defaults
  ...
  is fine when LiveCourse and ComfyUI share a host.
- **ComfyUI on a different machine in production:** point LiveCourse at ComfyUI
...
- **Disable prompt enhancement** — if your workflow has an LLM-based prompt
  ...
  per image. The prompts generated by LiveCourse are already descriptive enough.
- The adapter randomises the KSampler seed on every generation automatically.
- Output dimensions are calculated from the aspect ratio requested by LiveCourse
