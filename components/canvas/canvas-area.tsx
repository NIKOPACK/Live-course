...
interface CanvasAreaProps extends CanvasToolbarProps {
  ...
  /** Route scene presentation mutations through an optional in-memory store. */
  readonly presentationOnly?: boolean;
  readonly presentationStore?: StageStore;
  /** Board-first classroom: no play hint, scene index, or click-to-play. */
  readonly classroomChrome?: boolean;
}
...
export function CanvasArea({
  ...
  const { t } = useI18n();
  const showControls = mode === 'playback' && !whiteboardOpen && !classroomChrome;
  const showPlayHint =
    showControls &&
    engineState !== 'playing' &&
  ...
  return (
    <div
      className={cn(
        ...
        classroomChrome ? 'lc-classroom-stage' : 'bg-gray-50 dark:bg-gray-900',
      )}
    >
      ...
      <div
        className={cn(
          ...
          classroomChrome
            ? cn('lc-classroom-stage', !classroomFillBoard && 'items-center justify-center')
            : cn(
                'items-center justify-center transition-colors duration-500',
                currentScene?.type === 'interactive'
                  ? 'bg-blue-50/30 p-3 dark:bg-blue-900/10'
                  : 'bg-gray-100/70 p-3 dark:bg-gray-950/70',
              ),
        )}
        style={{ containerType: 'size' }}
      >
        <div
          data-testid={classroomChrome ? 'classroom-board' : undefined}
          className={cn(
            'relative overflow-hidden',
            classroomChrome
              ? cn(
                  'lc-classroom-board',
                  classroomFillBoard ? 'size-full' : 'aspect-[16/9] max-h-full',
                )
              : 'aspect-[16/9] max-h-full rounded-md bg-white shadow-lg ring-1 ring-black/5 transition-all duration-500 dark:bg-gray-900 dark:ring-white/10',
        ...
        >
          ...
          {/* Scene Number Badge — player chrome; omitted in the classroom. */}
          {currentScene && !classroomChrome && (
