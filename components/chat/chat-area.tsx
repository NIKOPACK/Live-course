...
interface ChatAreaProps {
  className?: string;
  /** Routes presentation/replay chat state away from canonical document chats. */
  presentationOnly?: boolean;
  /** Stage dependency used by replay action execution (must be non-persisting). */
  presentationStore?: StageStore;
  /** Optional explicit override; presentation-only mode always wins. */
  persistSessions?: boolean;
  /** Renders the record surface inside another workspace rail. */
  embedded?: boolean;
  width?: number;
...
export interface ChatAreaRef {
  ...
  /** Returns the lecture buffer pause state, or null when the session has no buffer. */
  getBufferPaused: (sessionId: string) => boolean | null;
...
export const ChatArea = forwardRef<ChatAreaRef, ChatAreaProps>(
  (
    ...
    const {
      ...
      shouldHoldAfterReveal,
      presentationOnly,
      persistSessions: persistSessions ?? !presentationOnly,
      stageStore: presentationStore,
    });
    ...
    const displayWidth = embedded ? '100%' : collapsed ? 0 : width;
    ...
    return (
      <div
        ...
        className={cn(
          'relative z-20 flex shrink-0 flex-col overflow-visible border-l border-gray-200 bg-white/90 shadow-[-2px_0_16px_rgba(0,0,0,0.03)] backdrop-blur-md dark:border-gray-800 dark:bg-gray-950/90',
          embedded && 'h-full border-0 bg-transparent shadow-none backdrop-blur-none',
          className,
        )}
      >
        {/* Drag handle */}
        {!embedded && !collapsed && (
          <div
            ...
            className="group absolute bottom-0 left-0 top-0 z-50 w-1.5 cursor-col-resize transition-colors hover:bg-purple-400/30 active:bg-purple-500/40 dark:hover:bg-purple-600/30 dark:active:bg-purple-500/40"
          >
