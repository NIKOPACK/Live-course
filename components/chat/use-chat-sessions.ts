...
export interface UseChatSessionsOptions {
  /**
   * Stage dependency used by whiteboard / widget action execution. Replay
   * passes its presentation-only adapter here so ActionEngine cannot route
   * mutations through the canonical persistence boundary.
   */
  stageStore?: StageStore;
  /** Keep chat sessions in the document's canonical `chats` field. */
  persistSessions?: boolean;
  /** Explicit replay/presentation mode. This always disables persistence. */
...
export type ChatSessionPersistenceOptions = Pick<
  UseChatSessionsOptions,
  'persistSessions' | 'presentationOnly'
>;

/**
 * Resolve the one persistence gate for chat session state.
 *
 * Presentation/replay state is deliberately fail-closed: even an accidental
 * `persistSessions: true` cannot write the canonical document while the
 * component is marked presentation-only.
 */
export function shouldPersistChatSessions(options: ChatSessionPersistenceOptions = {}): boolean {
  return options.presentationOnly !== true && options.persistSessions !== false;
}

/**
 * Build the hook's initial session state. Replay must not restore the old
 * teaching W from canonical `chats`; it starts with an in-memory session list
 * that playback can populate with the current lecture.
 */
export function getInitialChatSessions(
  storedSessions: ChatSession[],
  options: ChatSessionPersistenceOptions = {},
): ChatSession[] {
  return shouldPersistChatSessions(options)
    ? normalizeStoredSessionsForRestore(storedSessions)
    : [];
}

/**
 * Small persistence seam kept outside React so the replay negative contract is
 * directly testable: when the gate is closed, the persistence callback is
 * never invoked.
 */
export function persistChatSessionsIfEnabled(
  sessions: ChatSession[],
  options: ChatSessionPersistenceOptions,
  persist: (sessions: ChatSession[]) => void,
): boolean {
  if (!shouldPersistChatSessions(options)) return false;
  persist(sessions);
  return true;
}
 ...
 */
async function buildFreshAgentLoopStoreState(
  stageStore: StageStore = useStageStore,
): Promise<AgentLoopStoreState> {
  const stateBeforeQuizRead = stageStore.getState();
  const quizResults = await buildQuizResultsForStoreState(
    stateBeforeQuizRead.scenes,
    stateBeforeQuizRead.currentSceneId,
  );
  const freshState = stageStore.getState();
  // StageStore is intentionally a narrow action-injection interface and does
  // not expose document-only outline state. The canonical store remains the
  // source for that optional request field; replay still receives the same
  // read-only document snapshot while all mutations use `stageStore` above.
  const outlines = useStageStore.getState().outlines;
  return {
...
export function useChatSessions(options: UseChatSessionsOptions = {}) {
  // A presentation-only hook must never silently fall back to the canonical
  // store for action execution. Keep one adapter instance for the hook's
  // lifetime when the caller does not provide its own adapter.
  const fallbackPresentationStoreRef = useRef<StageStore | null>(null);
  if (options.presentationOnly && !options.stageStore && !fallbackPresentationStoreRef.current) {
    fallbackPresentationStoreRef.current = createStagePresentationStore();
  }
  const sessionStageStore =
    options.stageStore ??
    (options.presentationOnly ? fallbackPresentationStoreRef.current! : useStageStore);
  const persistSessions = shouldPersistChatSessions(options);
  ...
  const [sessions, setSessions] = useState<ChatSession[]>(() => {
    // Restore sessions from the canonical document only for teaching mode.
    // Replay/presentation starts with an empty in-memory W by contract.
    const stored = persistSessions ? useStageStore.getState().chats : [];
    return getInitialChatSessions(stored, options);
  });
  ...
  const persistenceModeRef = useRef(persistSessions);
  useEffect(() => {
    const stageChanged = stageId !== stageIdRef.current;
    const persistenceModeChanged = persistSessions !== persistenceModeRef.current;
    if (!stageChanged && !persistenceModeChanged) return;
    stageIdRef.current = stageId;
    persistenceModeRef.current = persistSessions;
    ...
    const stored = persistSessions ? useStageStore.getState().chats : [];
    setSessions(
      getInitialChatSessions(stored ?? [], {
        persistSessions,
        presentationOnly: !persistSessions,
      }),
    );
  ...
  }, [persistSessions, sessionStageStore, stageId]);
  ...
  useEffect(() => {
    persistChatSessionsIfEnabled(
      sessions,
      { persistSessions, presentationOnly: !persistSessions },
      (nextSessions) => {
        const canonicalState = useStageStore.getState();
        if (stageIdRef.current && stageIdRef.current === canonicalState.stage?.id) {
          canonicalState.setChats(nextSessions);
        }
      },
    );
  }, [persistSessions, sessionStageStore, sessions]);
      ...
      const buffer = new StreamBuffer(
        {
          ...
          async onActionReady(messageId: string, data: ActionItem, signal: AbortSignal) {
            ...
            try {
              const actionEngine = new ActionEngine(sessionStageStore);
              const action = {
                id: data.actionId,
  ...
  const runAgentLoopFn = useCallback(
    async (
      ...
      if (isPiChatEnabled()) {
        ...
        // without storeState.
        const storeState = await buildFreshAgentLoopStoreState(sessionStageStore);
      ...
      const outcome = await runAgentLoop(
        ...
        {
          getStoreState: () => buildFreshAgentLoopStoreState(sessionStageStore),
  ...
  const startLecture = useCallback(
    async (sceneId: string): Promise<string> => {
      ...
      const scene = sessionStageStore.getState().scenes.find((s) => s.id === sceneId);
  ...
  const getBufferPaused = useCallback((sessionId: string): boolean | null => {
    const buf = buffersRef.current.get(sessionId);
    return buf ? buf.paused : null;
  }, []);
