...
export function EditChromeRoot({ scene, isEditable, onToggleEditMode }: EditChromeRootProps) {
  ...
  useEffect(() => {
    document.body.dataset.livecourseEditor = 'true';
    return () => {
      delete document.body.dataset.livecourseEditor;
    };
  }, []);
  ...
  const headerControls = (
    <HeaderControls
      mode="edit"
      canEdit={isEditable}
      onToggleEditMode={isLiveCourseEditorEnabled() ? onToggleEditMode : undefined}
    />
  );
