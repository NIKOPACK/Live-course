...
export function MediaPopover({ onSettingsOpen }: MediaPopoverProps) {
  ...
  const enabledMap: Record<TabId, boolean> = {
    ...
    tts: isLiveCourseTTSEnabled() && ttsEnabled,
    asr: asrEnabled,
  };
  ...
  const enabledCount = [
    ...
    isLiveCourseTTSEnabled() && ttsEnabled,
    asrEnabled,
  ...
  const handleOpenChange = (isOpen: boolean) => {
    setOpen(isOpen);
    if (isOpen) {
      const first = (['image', 'video', 'asr'] as TabId[]).find((id) => enabledMap[id]);
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {isLiveCourseTTSEnabled() && ttsEnabled && <Volume2 className="size-3.5" />}
      ...
      <PopoverContent align="start" side="bottom" avoidCollisions={false} className="w-80 p-0">
        ...
        <div className="p-2 pb-0">
          <div className="flex gap-0.5 p-0.5 bg-muted/60 rounded-lg">
            {TABS.filter((tab) => tab.id !== 'tts' || isLiveCourseTTSEnabled()).map((tab) => {
        ...
        <div className="p-3 pt-2.5">
          ...
          {isLiveCourseTTSEnabled() && activeTab === 'tts' && (
            <TabPanel
              icon={Volume2}
