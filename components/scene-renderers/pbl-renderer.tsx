...
import { PBLPresentationView } from './pbl/v2/presentation';
...
interface PBLRendererProps {
  ...
  /** Replay/presentation renders are forbidden from mutating the document. */
  readonly presentationOnly?: boolean;
  readonly presentationStore?: StageStore;
}

export function PBLRenderer({
  content,
  mode: _mode,
  sceneId,
  ...
  // The canonical scene remains the document source of truth. Presentation
  // uses the resolved persisted snapshot directly; it does not need a second
  // local runtime projection because its static surface has no write path.
  const projectForRender = resolvedProjectV2;
  const handleProjectChange = useCallback(
    (next: PBLProjectV2) => {
      if (presentationOnly) {
        // This callback is intentionally unreachable from PBLPresentationView.
        // Keep the guard for an accidental future caller: replay must never
        // turn a presentation update into a document write.
        return;
      }
    ...
    [content, presentationOnly, sceneId],
  );

  // Replay/presentation is a read-only projection of persisted course data.
  // Keep this branch before the live PBL container so no workspace, chat,
  // submission panel, SSE effect, or completion callback can be mounted.
  // Hooks above remain unconditional, preserving hook order if the mode ever
  // changes without unmounting the renderer.
  if (presentationOnly) {
    return projectForRender ? (
      <PBLPresentationView project={projectForRender} />
    ) : (
  ...
  if (projectForRender) {
    return (
      <PBLV2Container
        sceneId={sceneId}
        projectV2={projectForRender}
        onProjectV2Change={handleProjectChange}
...
function PBLV2Container({
  ...
  // live inside whatever element is natively fullscreened — LiveCourse fullscreens
  ...
  // is on. NOTE: this never reads/writes LiveCourse code — it only chooses this
  ...
  // True while the browser is in native (OS) fullscreen — LiveCourse enters it
  // on the stage via `stageRef.requestFullscreen()`. In that mode LiveCourse's
  ...
  const handleWorkspaceProjectChange = useCallback(
    (next: PBLProjectV2) => {
      ...
      const store = presentationStore ?? useStageStore;
      const liveContent = store.getState().scenes.find((s) => s.id === sceneId)?.content as
        | PBLContent
        | undefined;
    ...
    [presentationOnly, presentationStore, sceneId, onProjectV2Change],
  );
    ...
    <div ref={hostRef} className="h-full w-full">
      ...
      {(runtimeProject.uiPhase === 'workspace' || runtimeProject.uiPhase === 'completed') &&
        ...
        createPortal(
          ...
          // launch), so LiveCourse's native fullscreen still shows the workspace.
  ...
  return (
    <>
      ...
      <motion.div
        ...
        transition={transition}
      >
        {/* Native (OS) fullscreen takes over the top-right slot: LiveCourse's
            own exit bar is hidden for PBL, so this is the only click target
            to leave true fullscreen (Esc still works, handled by LiveCourse).
            `document.exitFullscreen()` triggers LiveCourse's own fullscreenchange
            cleanup (keyboard unlock etc.) — we never touch LiveCourse code. The
            violet, borderless-ish treatment mirrors LiveCourse's native control
