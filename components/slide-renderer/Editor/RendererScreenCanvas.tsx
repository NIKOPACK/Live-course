...
import { SlideCanvas, type SlideEffects } from '@livecourse/renderer';
import type { PPTImageElement, PPTVideoElement } from '@livecourse/dsl';
...
function PlaybackVideoContent({
  ...
  // Replay is a read-only presentation surface. Keep the guard next to the
  // derived affordance as well as in the click handler so a future renderer
  // change cannot accidentally expose a write path.
  const canRetry = !presentationOnly && mediaResolutionCanRetry(media?.resolution);
...
export interface RendererScreenCanvasProps {
  readonly presentationOnly?: boolean;
}

export function RendererScreenCanvas({ presentationOnly = false }: RendererScreenCanvasProps) {
  ...
  return (
    <SlideCanvas
      ...
      videoInteractive={!presentationOnly}
      chrome
    />
  );
}
