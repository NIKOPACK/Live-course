...
import type { PPTElement, SlideBackground } from '@livecourse/dsl';
...
export function ScreenCanvas({ presentationOnly = false }: ScreenCanvasProps) {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden select-none" ref={canvasRef}>
      <div
        className="absolute overflow-hidden transition-transform duration-700 motion-reduce:transition-none"
        style={{
      ...
      >
        {/* Background layer */}
        <div className="w-full h-full bg-position-center" style={{ ...backgroundStyle }}></div>
...
export function PlaybackScreenCanvas({ presentationOnly = false }: ScreenCanvasProps) {
  return isPlaybackRendererEnabled() ? (
    <RendererScreenCanvas presentationOnly={presentationOnly} />
  ) : (
    <ScreenCanvas presentationOnly={presentationOnly} />
  );
}
