...
import type { Slide, PPTImageElement, PPTVideoElement } from '@livecourse/dsl';
import { SlideCanvas } from '@livecourse/renderer';
...
interface SlideThumbnailProps {
  ...
  /** Hide retry affordances when the thumbnail is mounted in replay. */
  readonly presentationOnly?: boolean;
}

/**
 * Read-only thumbnail rendering for a video element. Replaces `@livecourse/renderer`'s
...
function renderThumbnailVideo(
  ...
  return (
    <>
      ...
      {!presentationOnly && mediaResolutionCanRetry(media?.resolution) ? (
        <button
          onClick={(event) => {
...
/**
 * Read-only slide thumbnail rendered via the extracted `@livecourse/renderer`
 ...
 * is intentionally untouched (`@livecourse/renderer` v1 is read-only; editing is v2).
 */
