...
import type { PPTVideoElement } from '@livecourse/dsl';
...
export interface BaseVideoElementProps {
  ...
  /** Hide controls and mutation affordances on replay's read-only surface. */
 ...
 */
export function BaseVideoElement({ elementInfo, presentationOnly = false }: BaseVideoElementProps) {
  const { t } = useI18n();
  ...
  const canRetry = !presentationOnly && mediaResolutionCanRetry(resolution);
