...
interface SceneSidebarProps {
  ...
  /** Keep read-only replay thumbnails from exposing media retry writes. */
  readonly presentationOnly?: boolean;
  /** Replay may expose only the nodes persisted in its taught range. */
  readonly isSceneSelectable?: (sceneId: string) => boolean;
}
...
export function SceneSidebar({
  ...
  return (
    <div
      ...
      className="absolute inset-y-0 left-0 z-20 flex max-w-[calc(100vw-2.5rem)] flex-col overflow-visible rounded-r-md border-r border-gray-200 bg-white/90 shadow-[2px_0_16px_rgba(0,0,0,0.03)] backdrop-blur-md dark:border-gray-800 dark:bg-gray-950/90 md:relative md:max-w-none md:shrink-0 md:rounded-none"
    >
      {/* Drag handle */}
      {!collapsed && (
        <div
          onMouseDown={handleDragStart}
          className="group absolute bottom-0 right-0 top-0 z-50 w-1.5 cursor-col-resize transition-colors hover:bg-purple-400/30 active:bg-purple-500/40 dark:hover:bg-purple-600/30 dark:active:bg-purple-500/40"
        >
      ...
      <div className={cn('flex flex-col w-full h-full overflow-hidden', collapsed && 'hidden')}>
        ...
        >
          {scenes.map((scene, index) => {
            ...
            const isSelectable = isSceneSelectable?.(scene.id) ?? true;
