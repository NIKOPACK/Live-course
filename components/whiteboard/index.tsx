...
interface WhiteboardProps {
  ...
  /** Replay/presentation can render the board but must not expose write tools. */
  readonly presentationOnly?: boolean;
  /** Optional in-memory adapter used by replay action effects. */
...
export function Whiteboard({
  ...
  const { t } = useI18n();
  const stageStore = presentationStore ?? useStageStore;
  const stage = useSyncExternalStore(
    stageStore.subscribe,
    () => stageStore.getState().stage,
    () => stageStore.getState().stage,
  );
  ...
  const stageAPI = useMemo(() => createStageAPI(stageStore), [stageStore]);

  /**
   * Reset the local clear lifecycle and its shared animation flag. When an
   * owner is supplied, an older async clear is only allowed to clean up its
   * own controller; this prevents a stale continuation from cancelling a
   * newer clear that has already started.
   */
  const resetClearState = useCallback((owner?: AbortController): boolean => {
    if (owner && clearAbortRef.current !== owner) return false;
  ...
  /** Invalidate an in-flight clear before changing its backing surface. */
  const invalidateClear = useCallback(() => {
    clearEpochRef.current += 1;
    const owner = clearAbortRef.current;
    if (owner) {
      owner.abort();
      resetClearState(owner);
      return;
    }
    // `whiteboardClearing` is shared with ActionEngine. Do not reset that
    // flag when this component has no clear of its own; mounting, unmounting,
    // or switching presentation surfaces must not interrupt an engine-owned
    // cascade animation.
    clearingRef.current = false;
  }, [resetClearState]);
  ...
  // Closing the overlay or changing its backing store invalidates an in-flight
  // clear. A delayed callback must never delete a whiteboard that belongs to a
  // different scene/session (or to the replay presentation projection).
  useEffect(() => {
    if (isOpen) return;
    invalidateClear();
  }, [invalidateClear, isOpen]);

  useEffect(() => {
    invalidateClear();
  }, [invalidateClear, presentationOnly, stageStore]);

  const handleClear = async () => {
    if (presentationOnly || !whiteboard || elementCount === 0 || clearingRef.current) return;
    clearingRef.current = true;
    const clearEpoch = ++clearEpochRef.current;
    const clearAbort = new AbortController();
    clearAbortRef.current = clearAbort;
    const capturedStageId = stage?.id;
    const capturedWhiteboard = whiteboard;
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        const finish = () => {
          ...
          clearAbort.signal.removeEventListener('abort', finish);
          resolve();
        };
        const timer = window.setTimeout(finish, animMs);
        clearAbort.signal.addEventListener('abort', finish, { once: true });
      });

      const isCurrentClear =
        mountedRef.current &&
        clearEpochRef.current === clearEpoch &&
        !clearAbort.signal.aborted &&
        clearAbortRef.current === clearAbort;
      if (!isCurrentClear) return;

      const currentState = stageStore.getState();
      const currentWhiteboard = currentState.stage?.whiteboard?.find(
        (candidate) => candidate.id === capturedWhiteboard.id,
      );
      // The object identity check protects a newer draw/update from being
      // overwritten by this clear, even when the stage id stayed the same.
      if (currentState.stage?.id !== capturedStageId || currentWhiteboard !== capturedWhiteboard) {
        return;
      }

      // Actually remove elements
      const result = stageAPI.whiteboard.delete(capturedWhiteboard.id);
      ...
      // Every exit (abort, stale surface, backing-store replacement, API
      // failure, and success) goes through the same owner-aware cleanup.
      resetClearState(clearAbort);
    }
  };
