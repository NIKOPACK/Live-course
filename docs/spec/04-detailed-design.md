...
- 改角色：预览为每个段维护 `waiting / generating / completed / failed`；只有失败段暴露「重试该段」。单段失败后继续处理其他段，不取消其已在途内容；重试沿用同一 segment identity / idempotency key，成功段不重新排队、不被覆盖
...
- 才新写：`lib/livecourse/lesson/html-presentation.ts` 负责主 Agent 的课程级视觉方向与自由 HTML 页面 prompt。教案 schema 增加可选 `presentation: { mode: 'html', visualStyle: string, coverPrompt?: string }`；视觉方向以自由文本描述配色、字体、构图、图形语言和动效，不引入另一套模板枚举。先定风格，再让节点 worker 读同一方向。风格失败显式报错。个别节点经一次补充仍缺设计则该节点省略 `design`，整课继续生成；空字符串教学要点从数组去掉后再校验，有剩余要点则保留该节点。`coverPrompt` 缺失或无效不得让视觉方向失败，生成封面时再 fallback。
...
- 主教案与逐节点 worker 使用模型 JSON 输出模式，持久化前仍按既有 schema 校验；生成边界可以剥离额外的模型元数据，不能因为多了无关字段丢掉核心字段完整的教案。缺少必要讲授内容仍需定向补齐，不能造空设计。
- 才新写（尽力审校）：`lesson/quality-review.ts` 对完整教案独立核对知识、条件与范围；`generation/reviewed-scene.ts` 联合核对 HTML、结构化题目与讲稿/动作。生成成功率优先于准确性，审校不是保存门禁。沿用独立 `classroom-review` 路由和新的调用上下文，尊重推理设置，不把原作者说明当作正确性证据。
- 适度审校：仅高置信度的实质问题值得至多一次局部优化，不追求满分，不按字数、时长或装饰评分。审校异常、格式错误、服务不可用、修复失败或复核仍不满意时，记录明确的非阻断警告并保留最近的结构有效材料，不再额外纠正审校报告或重做整段。缺失结论不伪造为“审校通过”；取消和未知程序错误仍传播。初始空内容、缺节点、无效结构与脚本语法校验独立保留。
...
- 动作错误可能依赖 HTML 状态，不能因审校标成 `actions` 就禁止修页面。局部修复同时检查未调用的脚本钩子、隐藏后代与无法操作的状态；若仅需改讲稿或改用已有显露目标，返回空补丁，原页面保持不变。显式 HTML 错误仍要求非空补丁。两条生成路径的讲稿与独立审校尊重推理配置，不套用页面作者的强制非推理保护。
...
- 生成内修复：教案只替换被指出的节点设计/教学主线，保留节点身份、范围、视觉方向和其他节点；课堂成品只修当前未保存草稿。HTML 修复后重新校验语法并基于修复后的页面生成讲稿；只有讲稿出错时不重做页面。修复不得通过删掉核心知识、例题或缩短解释来过关。最多一次定向修复，之后只复核原问题及修复引入的明显严重回归，不扩大审校范围；复核仍有知识问题时记录警告并继续，不触发外层自动重复修复。修复生成了坏结构时弃用该修复，保留原来结构有效的材料。结构化测验题目及判分键不能被页面修复改写；无法在本次权限内修正的知识问题同样记录警告，不阻断生成。取消贯穿审校和修复。已有课堂和已成功段不进入此流程，不新增改课入口。
...
- 生成后进入课堂：API 生成的服务端文档可能尚无当前学习者的 `C`。普通课堂加载在挂载教学运行时前沿用 `initializeCourseState` 幂等补齐缺失的 `C`；已有状态、证据与进度不重置。该步骤也覆盖已缓存到本地的 API 课堂，严格沿用文档中的课程/课次身份；只读再听不初始化或写入状态，旧加载被切换后不得更新新课堂的投影。初始化失败显式停留在加载错误态。
