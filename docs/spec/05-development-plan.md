...
- 任一 subagent 失败时仅对缺失节点做一次单调用补充，不覆盖成功节点；补充后仍缺的节点省略 design，不挡整课开跑
...
教学质量验收：新课的教学主线随教案持久化；时长估算可选且不作为生成门槛，不因长讲稿额外调用模型缩写、截断或判失败。页面与动作收到同一节点设计，主讲与补充材料分开，不能宣称高亮动作改变控件状态。个别节点经定向补充仍缺设计时省略该节点 design 并继续逐段生成；补充不覆盖成功节点。浏览器/服务端的教案与讲稿独立路由、推理配置与模型原生输出窗口一致，显式关闭可用，空最终答案或截断输出不能冒充完整教案/讲稿，HTML 页面保持兼容保护；旧课无新字段仍可读取，生成重试不改成功段。回归覆盖充分展开的长讲稿、可选时长、失败恢复及推理输出完整性。

成功率优先验收：教案和最终 HTML/讲稿的独立审校只做尽力优化；坏审校 JSON、错误审校目标、服务不可用或修复耗尽均记录警告并保留结构有效的课件，不再使整课失败，也不伪造“审校通过”。高置信度实质问题至多修复一次，纯讲稿问题保持页面不变，页面状态问题允许局部 HTML 补丁。取消、未知程序错误和初始材料结构/脚本错误仍按原路径处理。覆盖一段失败后后续段继续生成、成功段不重做、只重试失败段，以及三课并发完整生成与页面/动作可运行；不能用部分课件或静默丢页冒充整课完成。
