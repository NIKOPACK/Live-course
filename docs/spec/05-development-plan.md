...
- 大知识树初始仅显示紧凑主题组，细项与长说明按需展开；父子独立勾选，收起不丢选择，总数和后代计数准确，确认提交仍包含隐藏已选项。总数、操作和保存失败提示保持可见，操作栏不得遮挡键盘焦点；覆盖键盘/触摸、长标题、窄屏、动态减少动效（含折叠途中切换，不得残留已关闭内容）及 busy/error 后展开状态保留
- 预览四种分段状态、正常生成或已完成段可点开只读查看教案与课堂材料、只重试失败段、成功段不重做、尚未生成完成的首页卡片回到预览、全部完成后显式进入课堂、课堂加载失败留在预览
- 预览阶段衔接：加载与确认共用页面框架，逐题变化与折叠详情无突然塌陷、文字缩放或按钮位移；流式大纲重试不反复切工作区，完整结果前不挂正式分段进度。材料步骤完成后仍可见，教案有独立当前态，排队不冒充生成，并发段分别保留子步骤。覆盖快速连续变化、手动展开状态、窄屏长标题、动态减少动效偏好；动效不改变生成请求、进度事实、重试及 W / C / L 边界
...
- J3.1 讲课手势仅在口型通道检测到真实发声时启动；覆盖连续讲授中的动作间隔、左右变化、停顿 / 暂停 / 打断 / 断开后的收回、再次发声、减少动态效果初始与动态切换、模型切换 / 卸载清理、缺骨骼 warning。验证待机与手势混合不累计旋转、口型和下半身不受影响，内置模型半身画面能看见手部动作；首页和无声连接不触发，手势不写 W / C / L、不发教学完成或证据事件
...
- 任一 subagent 失败时仅对缺失节点做一次单调用补充，不覆盖成功节点；新课补充后仍缺设计则留在预览重试，不以骨架冒充完整教案
...
脚本校验验收：完整 HTML 外壳内的缺括号、引号/逗号错误和截断脚本不得成为完成段；classic/module、事件属性与非执行数据脚本分别处理，校验不执行代码。覆盖一次定向修复成功、修复后仍无效、取消、后处理与注入桥脚本；修复不改结构化题目或成功段，耗尽不触发外层无限/倍增修复。旧页运行失败的保留作答、重试与显式基础题目恢复不变。

教学质量验收：新课的教学主线随教案持久化；时长估算可选且不作为生成门槛，不因长讲稿额外调用模型缩写、截断或判失败。页面与动作收到同一节点设计，主讲与补充材料分开，不能宣称高亮动作改变控件状态。任何节点经定向补充仍缺设计时教案明确失败；补充不覆盖成功节点。浏览器/服务端的教案与讲稿独立路由、推理配置与模型原生输出窗口一致，显式关闭可用，空最终答案或截断输出不能冒充完整教案/讲稿，HTML 页面保持兼容保护；旧课无新字段仍可读取，生成重试不改成功段。回归覆盖充分展开的长讲稿、可选时长、失败恢复及推理输出完整性。

上线审校验收：教案和最终 HTML/讲稿都有独立审校调用；坏审校 JSON、错误审校目标、取消、配置失败或修复耗尽不能返回成功。小瑕疵及非高置信度意见直接放行，不增加生成调用；只有明确硬伤触发一次定向修复，复核不扩大范围。教案修复保留未受影响节点，HTML 修复重做当前讲稿但不改题目/判分键；纯讲稿修复不调用页面模型。真实样本须覆盖已发现的正弦周期/振幅/范数混淆、积分区间与归一化，以及另一学科的解释与交互；用事实核对而非模型自评分验收。两条生成路径保存修复后的成品，失败不能写课堂或解锁进入；浏览器渲染、动作目标、测验提交与失败段恢复保持可用。不宣称审校能保证所有知识永远正确。

发布配置需确认 `classroom-review` 指向实际可用的服务端模型：复杂数学/代码建议独立的强模型，不能仅靠增加提示词假定快速模型具备同等修复能力。若使用 OpenAI 模型，必须先配置服务端 OpenAI 凭据/端点，再合并路由配置；不可只添加路由而缺失供应商配置。`lesson-plan` 与 `scene-actions` 的显式关闭仍受尊重，开启推理需同步改对应路由。教案和动作接口均为联合审校及单次修复留出 600 秒平台运行窗口，不引入讲稿字数/时长配额。
...
约束：封面不是教案节点配图，不进 `mediaGenerations`；subagent 无工具；不新增图片 provider；失败或未配置不中断备课；上游超时（含 524）再发一次生成请求后再判定失败；学习者不能改封面。
...
- 仅列表 `generationComplete === true`、非展示课的卡片有分享列；未完成落盘课无列；取消不改数据；401/缺媒体/413 可重试。页内 `url(#id)` 不算缺媒体
