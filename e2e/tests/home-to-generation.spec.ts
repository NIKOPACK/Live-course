...
test.describe('Home → Generation', () => {
  ...
  test('shows the required hint on blur and keyboard submit attempts', async ({ page }) => {
    const home = new HomePage(page);
    await home.goto();

    await home.textarea.focus();
    await home.textarea.press('Tab');
    await expect(page.getByText('Please enter course requirements')).toBeVisible();
    await expect(home.textarea).toHaveAttribute('aria-invalid', 'true');

    await home.fillRequirement('Linear algebra');
    await expect(page.getByText('Please enter course requirements')).toBeHidden();

    await home.fillRequirement('');
    await home.textarea.press('Control+Enter');
    await expect(page.getByText('Please enter course requirements')).toBeVisible();
