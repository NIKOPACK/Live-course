 ...
 *          the user sets an explicit baseUrl) → generate button disabled, a
 *          Settings hint outside the prompt, no model picker in the composer.
 *          NO modelNotConfigured toast, NO forced settings dialog.
 * State B: a server-configured provider → a concrete model is auto-resolved,
 *          the composer has no model chrome, and generation is enabled.
 *          Channel / key / model selection stays in Settings.
 *
...
test.describe('#580 model-selection invariant', () => {
  test('State A: no usable provider → disabled generate + single Set-up affordance, no toast', async ({
    ...
    await expect(page.getByTestId('configure-model-hint')).toBeVisible();
    await expect(
      page.getByTestId('home-prompt-toolbar').getByText(SETUP_CTA, { exact: true }),
    ).toHaveCount(0);
    await expect(page.getByTestId('home-model-picker')).toHaveCount(0);
  ...
  test('State B: server-configured provider → concrete model auto-selected, generation enabled', async ({
    ...
    await expect(page.getByTestId('home-model-picker')).toHaveCount(0);
    await expect(page.getByTestId('configure-model-hint')).toHaveCount(0);
    await expect(
      page.getByTestId('home-prompt-toolbar').getByText(SETUP_CTA, { exact: true }),
    ).toHaveCount(0);
    await expect(page.locator('button[aria-label*=" / "]')).toHaveCount(0);
