...
/**
 * The settings store persists through the `@livecourse/storage` KVStore, and does
...
test.describe('settings persistence through the KVStore', () => {
  ...
  test('loads a store already living in the KV scope, with no raw key present', async ({
    ...
    // Hydration read the KV scope: a usable provider enables generation, and
    // the composer does not carry model chrome.
    ...
    await expect(home.enterButton).toBeEnabled({ timeout: 15_000 });
