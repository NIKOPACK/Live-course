...
const eslintConfig = defineConfig([
  ...
  globalIgnores([
    ...
    'packages/pptxgenjs/**',
    // Our own @livecourse/* packages: lint the source, but skip build output,
    // installed deps, and the vendored JS sources under importer/src1.
    'packages/@livecourse/*/dist/**',
    'packages/@livecourse/*/node_modules/**',
    'packages/@livecourse/importer/src1/**',
    ...
    'render-service/**',
    // Local user capture scripts and scratch artifacts (tmp/shots/*.js etc.) —
    // pre-existing, untracked, not repository source. Never staged or linted.
    'tmp/**',
  ]),
  {
    rules: {
  ...
  // Package boundary (machine-enforced): @livecourse/renderer is a standalone,
  ...
  // isolation (only `@livecourse/dsl` + declared peers external), not by this rule.
  {
    files: ['packages/@livecourse/renderer/**/*.{ts,tsx,js,jsx,mjs,cjs}'],
    rules: {
      'no-restricted-syntax': [
        ...
        ...AI_SDK_DYNAMIC_IMPORT_BAN,
        {
          ...
          message:
            '@livecourse/renderer must not reference a host-app path (@/…). This package authors no `@/…` strings — depend only on @livecourse/dsl and declared peers, and inject host concerns (stores, undo, media resolution, i18n, hotkeys) via props/callbacks.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@livecourse/renderer must not reference a host-app path (@/…) in a template literal. Depend only on @livecourse/dsl and declared peers; inject host concerns via props/callbacks.',
        },
      ],
    },
  },
  // Package boundary (machine-enforced): @livecourse/storage is a standalone,
  // app-agnostic persistence package. Same policy as the @livecourse/renderer
  ...
  // It depends only on @livecourse/dsl; host wiring (which store persists where)
  // lives in the app, which imports the package, never the reverse.
  {
    files: ['packages/@livecourse/storage/**/*.{ts,tsx,js,jsx,mjs,cjs}'],
    rules: {
      'no-restricted-syntax': [
        'error',
        ...AI_SDK_DYNAMIC_IMPORT_BAN,
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            '@livecourse/storage must not reference a host-app path (@/…). This package authors no `@/…` strings — depend only on @livecourse/dsl. The app wires its stores through the package, not the reverse.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@livecourse/storage must not reference a host-app path (@/…) in a template literal. Depend only on @livecourse/dsl.',
        },
      ],
    },
  },
  // Package boundary (machine-enforced): @livecourse/generation is a standalone,
  ...
  // with package.json.
  {
    files: ['packages/@livecourse/generation/**/*.{ts,tsx,js,jsx,mjs,cjs}'],
    rules: {
      'no-restricted-syntax': [
        'error',
        ...AI_SDK_DYNAMIC_IMPORT_BAN,
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            '@livecourse/generation must not reference a host-app path (@/…). Depend only on @livecourse/dsl, Node built-ins, and relative modules.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@livecourse/generation must not reference a host-app path (@/…) in a template literal.',
        },
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|(jsonrepair|katex|nanoid|partial-json)(\\/|$)|node:|\\.\\.?\\/).+/]',
          message:
            '@livecourse/generation may import only from @livecourse/dsl, approved leaf runtime dependencies, Node built-ins, or relative modules (./… or ../…).',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|(jsonrepair|katex|nanoid|partial-json)(\\/|$)|node:|\\.\\.?\\/).+/]',
          message:
            '@livecourse/generation may re-export only from @livecourse/dsl, approved leaf runtime dependencies, Node built-ins, or relative modules (./… or ../…).',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|(jsonrepair|katex|nanoid|partial-json)(\\/|$)|node:|\\.\\.?\\/).+/]',
          message:
            '@livecourse/generation may re-export only from @livecourse/dsl, approved leaf runtime dependencies, Node built-ins, or relative modules (./… or ../…).',
        },
        {
          ...
          message:
            '@livecourse/generation must use static imports from @livecourse/dsl, Node built-ins, or relative modules.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message: '@livecourse/generation must not use require().',
        },
      ],
    },
  },
  ...
  {
    files: [
      'packages/@livecourse/generation/test/**/*.{ts,tsx,js,jsx,mjs,cjs}',
      'packages/@livecourse/generation/vitest.config.ts',
    ],
    rules: {
      'no-restricted-syntax': [
        'error',
        ...AI_SDK_DYNAMIC_IMPORT_BAN,
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            '@livecourse/generation tests and test config must not reference a host-app path (@/…). Read app prompt assets as files for parity checks.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@livecourse/generation tests and test config must not reference a host-app path (@/…) in a template literal.',
        },
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|@livecourse\\/generation(\\/|$)|node:|vitest(\\/|$)|\\.\\.?\\/).+/]',
          message:
            '@livecourse/generation tests and test config may import only @livecourse/dsl, the package public entry, Node built-ins, Vitest, or relative modules (./… or ../…).',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|@livecourse\\/generation(\\/|$)|node:|vitest(\\/|$)|\\.\\.?\\/).+/]',
          message:
            '@livecourse/generation tests and test config may re-export only @livecourse/dsl, the package public entry, Node built-ins, Vitest, or relative modules (./… or ../…).',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|@livecourse\\/generation(\\/|$)|node:|vitest(\\/|$)|\\.\\.?\\/).+/]',
          message:
            '@livecourse/generation tests and test config may re-export only @livecourse/dsl, the package public entry, Node built-ins, Vitest, or relative modules (./… or ../…).',
        },
        {
          selector: 'ImportExpression',
          message: '@livecourse/generation tests and test config must use static imports.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message: '@livecourse/generation tests and test config must not use require().',
        },
      ],
    },
  },
  ...
  //      on @livecourse/dsl (types + the fire-and-forget partition) and relative
    ...
    rules: {
      'no-restricted-syntax': [
        'error',
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            'lib/choreography must not reference a host-app path (@/…). It authors no `@/…` strings — depend only on @livecourse/dsl and relative siblings, so the exporter can interpret it in pure Node. The app and exporter import it, not the reverse.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            'lib/choreography must not reference a host-app path (@/…) in a template literal. Depend only on @livecourse/dsl and relative siblings.',
        },
        // Import allowlist (static imports/re-exports): the ONLY permitted
        // sources are `@livecourse/dsl`(/subpaths), `zod`, and in-folder relatives
        ...
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/).+/]',
          message:
            'lib/choreography may import only from @livecourse/dsl, zod, or in-folder relatives (./…). No parent-escape (../…) into the app and no other packages — keep it pure so the exporter runs in plain Node.',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/).+/]',
          message:
            'lib/choreography may re-export only from @livecourse/dsl, zod, or in-folder relatives (./…).',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/).+/]',
          message:
            'lib/choreography may re-export only from @livecourse/dsl, zod, or in-folder relatives (./…).',
        },
        ...
        // can pull in a render backend at runtime.
        {
          selector: 'ImportExpression',
          message:
            'lib/choreography must not use dynamic import() — it bypasses the static import allowlist. Use a top-level import from @livecourse/dsl, zod, or a relative sibling.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message:
    ...
    rules: {
      'no-restricted-syntax': [
        'error',
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            'lib/video-export must not reference a host-app path (@/…). Live app state enters only through the injected TimingProbe / AssetSource — depend only on @livecourse/dsl, zod, ../choreography, and relative siblings, so the compiler runs in pure Node.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            'lib/video-export must not reference a host-app path (@/…) in a template literal. Depend only on @livecourse/dsl, zod, ../choreography, and relative siblings.',
        },
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/choreography(\\/|$)).+/]',
          message:
            'lib/video-export root files may import only from @livecourse/dsl, zod, ../choreography, or in-module children (./…). A ../… into anything but ../choreography escapes the module and is rejected.',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/choreography(\\/|$)).+/]',
          message:
            'lib/video-export root files may re-export only from @livecourse/dsl, zod, ../choreography, or in-module children (./…).',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/choreography(\\/|$)).+/]',
          message:
            'lib/video-export root files may re-export only from @livecourse/dsl, zod, ../choreography, or in-module children (./…).',
        },
        {
          selector: 'ImportExpression',
          message:
            'lib/video-export must not use dynamic import() — it bypasses the static import allowlist. Use a top-level import from @livecourse/dsl, zod, ../choreography, or a relative sibling.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message:
      ...
      'no-restricted-syntax': [
        ...
        // `../../…` escape is rejected UNLESS it is exactly `../../choreography`.
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/\\.\\.\\/choreography(\\/|$)|\\.\\.\\/(?!\\.\\.\\/)).+/]',
          message:
            'lib/video-export passes may import only from @livecourse/dsl, zod, ../../choreography, or in-module relatives (./… or a single ../… that stays inside the module). A ../../ escape into the rest of the app is rejected.',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/\\.\\.\\/choreography(\\/|$)|\\.\\.\\/(?!\\.\\.\\/)).+/]',
          message:
            'lib/video-export passes may re-export only from @livecourse/dsl, zod, ../../choreography, or in-module relatives.',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@livecourse\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/\\.\\.\\/choreography(\\/|$)|\\.\\.\\/(?!\\.\\.\\/)).+/]',
  ...
  // than by an argument:
  //   - packages/@livecourse/renderer, packages/@livecourse/storage, and
  //     packages/@livecourse/generation — the same
  ...
  //     against @livecourse/dsl; review showed `void import('ai')` under the renderer
  ...
  //     ImportExpression outright, which subsumes this one.
  {
    ...
    ignores: [
      ...
      'packages/@livecourse/renderer/**',
      'packages/@livecourse/storage/**',
      'packages/@livecourse/generation/**',
    ],
    rules: {
