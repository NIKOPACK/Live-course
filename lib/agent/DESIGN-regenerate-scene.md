# LiveCourse Editor Agent — `regenerate_scene` (next-release capability widening)

- Date: 2026-06-21
- Base branch: `feat/livecourse-editor-agent-v0`
- Status: design approved, pending spec review → implementation plan

## 1. Background

The LiveCourse Editor Agent v0 (`feat/livecourse-editor-agent-v0`) stands up a server-side
...
- The existing `aiCall` (resolved `livecourse-agent` model) is shared by all tools.
...
- Master gate unchanged: `isLiveCourseEditorEnabled()` 404s the route when off.
