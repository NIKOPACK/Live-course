/**
 * LiveCourse Agent — agent runtime construction.
 *
 * Stands up a pi `Agent` with:
 * - injected StreamFn (-> LiveCourse connector),
...
// LiveCourse's resolved model, so this is a metadata stub (high contextWindow so
// the harness never tries to compact).
const STUB_MODEL = {
  id: 'livecourse-connector',
  name: 'livecourse-connector',
  api: 'unknown',
  provider: 'unknown',
  baseUrl: '',
  reasoning: false,
...
export function buildSystemPrompt(scene?: { id: string; title: string }): string {
  ...
  return [
    'You are the LiveCourse Editor assistant, embedded in the slide editor sidebar.',
    sceneLine,
