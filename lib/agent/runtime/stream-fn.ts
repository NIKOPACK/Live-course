/**
 * LiveCourse Agent — pi StreamFn adapter (promoted from PoC).
 *
 * Bridges the pi agent loop's LLM call to LiveCourse's existing AI-SDK-based
 ...
 * call through LiveCourse's resolved Vercel `LanguageModel`, then map the AI SDK
...
/** Build a pi `StreamFn` that calls LiveCourse's connector instead of pi-ai providers. */
