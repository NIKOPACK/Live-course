...
function createConnectorModel(api: Api, contextWindow: number, maxTokens: number): Model<Api> {
  return {
    id: 'livecourse-director-compaction',
    name: 'LiveCourse Director Compaction',
    api,
    provider: 'livecourse-connector',
    baseUrl: '',
    reasoning: false,
    input: ['text'],
...
export function createDirectorCompactionRuntime(opts: {
  ...
  const sourceId = `livecourse-director-compaction:${nanoid()}`;
