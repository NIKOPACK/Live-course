...
export interface ClassroomPayload {
  stage: Stage;
  scenes: Scene[];
  /** Optional versioned course metadata returned by a server-backed copy. */
  coursePlan?: CoursePlan;
  /** A1 persisted teaching design; omitted on legacy classrooms. */
...
export interface RunClassroomLoadArgs<TMediaTasks = unknown> {
  classroomId: string;
  /** True for replay's presentation-only document load. */
  readOnly?: boolean;
...
export async function runClassroomLoad<TMediaTasks = unknown>({
  ...
  try {
    if (readOnly) {
      await loadFromStorage(classroomId, loadToken, { readOnly: true });
    } else {
      // Preserve the historical two-argument call shape for normal teaching
      // loads; a few integrations use the function as a narrow adapter.
    ...
    if (!getCurrentStage()) {
      ...
      if (classroom) {
        const { stage, scenes, coursePlan, lessonPlan } = classroom;
...
export async function fetchClassroomFromApi(classroomId: string): Promise<ClassroomPayload | null> {
  ...
  // The server fallback is an untrusted wire boundary. Preserve an omitted
  // plan for legacy classrooms, but never let a malformed plan enter the
  // in-memory store and later masquerade as a legacy course.
  const coursePlan =
    classroom.coursePlan === undefined ? undefined : parseCoursePlan(classroom.coursePlan);
  const lessonPlan =
    classroom.lessonPlan === undefined ? undefined : lessonPlanSchema.parse(classroom.lessonPlan);
  return {
    ...classroom,
    ...(coursePlan === undefined ? {} : { coursePlan }),
    ...(lessonPlan === undefined ? {} : { lessonPlan }),
