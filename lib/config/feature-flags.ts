...
/**
 * LiveCourse currently uses OpenAI Realtime as its only teacher-audio path.
 *
 * Keep the inherited LiveCourse TTS implementation intact for a future product
 * decision, but do not let persisted settings, configured providers, or cached
 * narration assets reactivate it accidentally. Re-enabling requires an explicit
 * deployment-time decision, not a browser setting.
 */
export function isLiveCourseTTSEnabled(): boolean {
  return readBoolean(process.env.NEXT_PUBLIC_ENABLE_LIVECOURSE_TTS);
}

/**
 * LiveCourse Editor (Pro mode) gate. Default OFF — gates only the Pro toggle
 ...
 * state.
 */
export function isLiveCourseEditorEnabled(): boolean {
  return readBoolean(process.env.NEXT_PUBLIC_LIVECOURSE_EDITOR_ENABLED);
}
...
export function isPlaybackRendererEnabled(): boolean {
  return readBoolean(process.env.NEXT_PUBLIC_LIVECOURSE_PLAYBACK_RENDERER_ENABLED);
}
...
export function isPiWebSearchEnabled(): boolean {
  return readBoolean(process.env.LIVECOURSE_ENABLE_PI_WEB_SEARCH);
}
...
export function isVocationalTaskEngineEnabled(): boolean {
  return readBoolean(process.env.LIVECOURSE_ENABLE_VOCATIONAL);
}
