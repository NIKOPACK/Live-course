import { DSL_VERSION, migrate } from '@livecourse/dsl';
import { BrowserKVStore, type DocumentStore, type KVStore } from '@livecourse/storage';
...
export function documentLockName(stageId: string): string {
  return `livecourse:document:${encodeURIComponent(stageId)}`;
}
 ...
 * This mirrors `@livecourse/storage`'s private `migrateDocument` in
