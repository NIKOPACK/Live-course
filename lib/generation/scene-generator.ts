...
export interface SceneContentOptions {
  ...
  /**
   * Natural-language edit instruction for whole-slide regeneration (LiveCourse Editor
  ...
  /** Lesson design is shared by every HTML page and the legacy slide generator. */
 ...
 * owned by the DSL contract — `normalizeElement` from `@livecourse/dsl` — rather
...
async function generateSlideContent(
  ...
  const prompts = buildPrompt(PROMPT_IDS.SLIDE_CONTENT, {
    ...
    // A1：教案节点讲授设计；无教案时为空串（loader 默认值），占位符不泄漏。
    lessonDesign: formatLessonNodeDesignForPrompt(lessonNodeDesign),
    imageElementEnabled,
  ...
  // EDIT MODE (LiveCourse Editor agent `regenerate_scene`): when an edit instruction
...
async function generateQuizContent(
  ...
  const response = await aiCall(
    prompts.system,
    [prompts.user, formatLessonNodeDesignForPrompt(lessonNodeDesign)].filter(Boolean).join('\n\n'),
  );
...
export async function generateSceneActions(
  ...
  if ('html' in content && !('questions' in content)) {
    outline = { ...outline, type: 'interactive' };
  }
  const agentsText = formatAgentsForPrompt(agents);

  // Debug: Log content type for interactive scenes
  if (outline.type === 'interactive') {
    const hasHtml = 'html' in content && !('questions' in content);
    log.info(
  ...
  if (outline.type === 'interactive' && 'html' in content && !('questions' in content)) {
    const config = outline.interactiveConfig;
    const agentsText = formatAgentsForPrompt(agents);
...
function processActions(
  ...
  return actions.map((action) => {
    ...
    // Learner path has one speaker (docs/spec/04 §1/§2). Fold leftover
    // classmate `discussion` actions into teacher speech instead of assigning
    // a student agentId.
    if (processedAction.type === 'discussion') {
      const topic = processedAction.topic.trim();
      const speech: Action = {
        id: processedAction.id,
        type: 'speech',
        text: topic,
        title: processedAction.title ?? topic,
      };
      return speech;
    }
