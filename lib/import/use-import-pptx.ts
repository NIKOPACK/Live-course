...
// pdfjs-dist (transitively pulled by `livecourse-importer/src`) uses dynamic
...
import type * as LiveCourseImport from '@livecourse/importer';

const log = createLogger('ImportPptx');

export type PptxUpload = NonNullable<LiveCourseImport.ImportPptxOptions['upload']>;
 ...
 * the bundled `livecourse-importer` dist that we load by URL to bypass
  ...
  const handleFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        // `scripts/sync-livecourse-importer.mjs` copies the prebuilt dist into
        // `public/vendor/` after every `pnpm install`.
        const url = '/vendor/livecourse-importer/index.js';
