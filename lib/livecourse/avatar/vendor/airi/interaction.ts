 ...
 * Upstream: packages/stage-ui-three/src/composables/vrm/interaction.ts
 * Framework-neutral already; only formatting differs.
 */
...
function getModelScale(vrm: VRM) {
  ...
  bounds.getSize(size);
  if (!Number.isFinite(size.y) || size.y <= 0) return 1;
...
export function getVrmInteractionTargetFromObjectName(name: string): VrmInteractionTarget | null {
  if (!name.startsWith(COLLIDER_NAME_PREFIX)) return null;
...
/** AIRI Stage.vue: pointer interactions map to emotions with a per-target cooldown. */
export const VRM_INTERACTION_COOLDOWN_MS = 450;

export function getVrmInteractionExpression(target: VrmInteractionTarget): string {
  if (target === 'head') return 'happy';
  if (target === 'leftFoot' || target === 'rightFoot') return 'relaxed';
  return 'surprised';
}
