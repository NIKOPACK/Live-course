 ...
 * Upstream: packages/stage-ui-three/src/composables/vrm/loader.ts
 * `useVRMLoader` Vue composable → `getAiriVrmLoader()` factory; the
 * `AiriMToonMaterialLoaderPlugin` outline-fallback logic is verbatim, with the
 * `AIRI_ALL_OUTLINE` compile-time flag exposed as a runtime option.
 */
...
type MToonExtension = NonNullable<ReturnType<MToonMaterialLoaderPlugin['_getMToonExtension']>>;
...
class AiriMToonMaterialLoaderPlugin extends MToonMaterialLoaderPlugin {
  ...
  private readonly _allOutline: boolean;

  constructor(
    parser: ConstructorParameters<typeof MToonMaterialLoaderPlugin>[0],
    allOutline: boolean = AIRI_ALL_OUTLINE,
  ) {
    super(parser);
    this._allOutline = allOutline;
  }

  override async beforeRoot() {
    await super.beforeRoot();

    if (!this._allOutline) return;
  ...
  override extendMaterialParams(materialIndex: number, materialParams: Record<string, unknown>) {
    ...
    if (!this._allOutline) return pending;
  ...
  private _hasEnabledOutline(extension: MToonExtension) {
    return (
  ...
  private _resolveOutlineFallbackSettings() {
    ...
    const widthFactorAverage =
      dominantModeWidthFactors.reduce((sum, value) => sum + value, 0) /
      dominantModeWidthFactors.length;
...
export interface AiriVrmLoaderOptions {
  /** AIRI's experimental outline fallback (`AIRI_ALL_OUTLINE` upstream). */
  allOutline?: boolean;
}

export function createAiriVrmLoader(options: AiriVrmLoaderOptions = {}): GLTFLoader {
  ...
  loader.register((parser) => {
    ...
    const mtoonMaterialPlugin = new AiriMToonMaterialLoaderPlugin(parser, options.allOutline);
