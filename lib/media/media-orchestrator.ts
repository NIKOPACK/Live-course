...
import type { AssetMeta, PPTElement, Slide } from '@livecourse/dsl';
...
function rewriteStageAndScenes(
  ...
  const rewrittenScenes = scenes.map((scene): Scene => {
    ...
    if (
      ...
      const html = replaceHtmlMediaReferences(scene.content.html, { [oldRef]: assetId });
      if (html !== scene.content.html) {
        changed = true;
        nextScene = { ...scene, content: { ...scene.content, html } } as Scene;
      }
    }
