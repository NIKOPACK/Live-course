import { BrowserKVStore, type KVStore, type RuntimeStore } from '@livecourse/storage';
import type { RuntimeRecord } from '@livecourse/dsl';
...
export interface HydratePBLProjectArgs {
  ...
  learnerKey?: string;
  /**
   * Read runtime state without draining project outboxes or repairing the
   * runtime/document cutover. Replay mounts use this boundary so merely
   * viewing a persisted lesson cannot create runtime sessions, append
   * backfill/self-heal snapshots, or update drain watermarks.
   */
  readOnly?: boolean;
}
...
/**
 * Fold already-persisted runtime records for a presentation-only read.
 *
 * The normal hydration path intentionally drains bounded project outboxes and
 * may append a backfill/self-heal snapshot. Those are valid teaching/document
 * maintenance operations, but they are writes and therefore forbidden during
 * replay. This helper only lists existing records and applies the same
 * document-vs-runtime authority rule without repairing either side.
 */
async function hydratePBLProjectReadOnly(
  ...
  // Before write cutover the embedded document remains authoritative. Keep it
  // intact rather than silently replacing it with a stale/partial runtime
  // fold. The normal path would append a repair snapshot here; replay must not.
...
export async function hydratePBLProjectFromRuntime(
  ...
  if (args.readOnly) {
    return withRuntimeStorageSharedLockUntilSettled(async () => {
      ...
      const learnerKey = args.learnerKey ?? (await getLearnerKey(kv));
      const store = args.store ?? getRuntimeStore();
      return hydratePBLProjectReadOnly(args, store, learnerKey);
    }, PBL_HYDRATION_DRAIN_BARRIER_TIMEOUT_MS);
  }
