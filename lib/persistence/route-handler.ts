...
import { PgDocumentStore, ensureDocumentSchema } from '@livecourse/storage/document/pg';
import { PgRuntimeStore, ensureSchema } from '@livecourse/storage/runtime/pg';
import { createStorageHttpHandler } from '@livecourse/storage/server';
import {
  nodePostgresTransaction,
  type ConnectableQueryable,
} from '@livecourse/storage/server/reference';
import { Pool } from 'pg';
...
const HANDLER_STATE_KEY = Symbol.for('livecourse.persistence-route.handler');
...
function getPersistenceHandler(
  ...
  const initialization = createPersistenceHandler(connectionString, poolFactory).catch((error) => {
    // A failed initialization must not poison the process-wide singleton.
