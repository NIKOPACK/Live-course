...
export class PlaybackEngine {
    ...
    for (let i = 0; i < actionIndex; i++) {
      ...
      if (isWhiteboardPlaybackAction(action)) {
        try {
          ...
          if (!this.callbacks.speak) throw error;
          this.failTeachingAction(error, { sceneIndex: 0, actionIndex: i }, generation);
          return false;
        }
      }
    }
  ...
  pause(): void {
    if (this.mode === 'playing') {
      // A delayed whiteboard/video action is not complete when pause is
      // requested. Restore its cursor before cancelling so resume replays the
      // action instead of silently skipping it.
      const activeAction = this.activeAction;
      if (activeAction && !activeAction.settled) {
        this.sceneIndex = activeAction.sceneIndex;
        this.actionIndex = activeAction.actionIndex;
      }
      if (this.activeSpeech) {
        this.sceneIndex = this.activeSpeech.sceneIndex;
        this.actionIndex = this.activeSpeech.actionIndex;
      }
      this.invalidatePlaybackGeneration();
      ...
      this.setMode('paused');
      if (this.callbacks.speak) return;
  ...
  /**
   * Update the whiteboard shell through the ActionEngine ownership seam.
   * Replay chrome uses this for an explicit close/toggle so a stale playback
   * instance cannot mutate a newer replay's Canvas projection.
   */
  setWhiteboardOpen(open: boolean): void {
    this.actionEngine.setWhiteboardOpen(open);
  }
  ...
  handleEndDiscussion(): void {
    ...
    this.actionEngine.setWhiteboardOpen(false);
  ...
  private invalidatePlaybackGeneration(): number {
    ...
    this.actionEngine.cancelPendingActions?.();
    return this.playbackGeneration;
  }

  private failTeachingAction(
    error: unknown,
    cursor: { sceneIndex: number; actionIndex: number },
    generation: number,
  ): void {
    if (!this.isCurrentGeneration(generation)) return;
    this.sceneIndex = cursor.sceneIndex;
    this.actionIndex = cursor.actionIndex;
    this.invalidatePlaybackGeneration();
    this.setMode('paused');
    const normalized = error instanceof Error ? error : new Error(String(error));
    if (this.callbacks.onError) this.callbacks.onError(normalized);
    else log.error('Teacher playback failed:', normalized);
  }

  private async playTeacherSpeech(
    text: string,
    cursor: { sceneIndex: number; actionIndex: number },
    generation: number,
  ): Promise<boolean> {
    ...
    if (!text.trim()) {
      this.failTeachingAction(
        new Error('Teaching speech has no narration text'),
        cursor,
        generation,
      );
      return false;
    ...
    switch (action.type) {
      ...
      case 'laser': {
        if (this.callbacks.speak) {
          ...
          try {
            await this.actionEngine.execute(action, { signal: activeAction.controller.signal });
          } catch (error) {
            this.failTeachingAction(error, actionCursor, generation);
            return;
          } finally {
      ...
      case 'discussion': {
        ...
        if (this.callbacks.speak) {
          if (await this.playTeacherSpeech(discussionAction.topic, actionCursor, generation)) {
            this.markDiscussionConsumed(discussionAction.id);
            this.processNext(generation);
          }
          return;
        }
        if (this.callbacks.skipRoundtableDiscussion) {
          this.markDiscussionConsumed(discussionAction.id);
          this.processNext(generation);
          return;
        }
