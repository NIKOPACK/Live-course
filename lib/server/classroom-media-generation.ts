...
export function replaceMediaPlaceholders(scenes: Scene[], mediaMap: Record<string, string>): void {
  ...
  const htmlMediaMap = Object.fromEntries(
    Object.entries(mediaMap).filter(([ref]) => isGeneratedMediaPlaceholder(ref)),
  );
  ...
  for (const scene of scenes) {
    if (
      ...
      scene.content.html = replaceHtmlMediaReferences(scene.content.html, htmlMediaMap);
      continue;
    }
