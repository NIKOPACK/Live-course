...
function applyOpenAIImageFallback(
  ...
  const enabledRaw = process.env.IMAGE_OPENAI_ENABLED;
  if (enabledRaw !== undefined && enabledRaw.trim() !== '' && !parseBooleanEnv(enabledRaw)) {
    delete imageConfig[OPENAI_IMAGE_PROVIDER_ID];
    return imageConfig;
  }
 ...
 * - resolveWebSearchApiKey(clientKey) -> Zhihu key resolution
...
export function resolveWebSearchApiKey(providerIdOrClientKey?: string, clientKey?: string): string {
  ...
  const providerId = hasProviderId ? providerIdOrClientKey || 'zhihu' : 'zhihu';
...
export function resolveServerWebSearchProviderId(preferredProviderId?: string): string | undefined {
  ...
  if (webSearch.zhihu?.apiKey) return 'zhihu';
...
/**
 * Full-duplex classroom voice (OpenAI Realtime / Volc). Env-only; keys never
 * leave the server. Presence is a managed flag so settings can hide the
 * credential fields when the operator already configured the class voice.
 */
function isOfficialOpenAIRealtimeEndpoint(baseUrl?: string): boolean {
...
export function getServerRealtimeProviders(): Record<string, Record<string, never>> {
  const result: Record<string, Record<string, never>> = {};
  // OPENAI_API_KEY may point at an OpenAI-compatible LLM gateway. Official
  // Realtime client-secrets only work against api.openai.com.
  if (
    process.env.OPENAI_API_KEY?.trim() &&
    isOfficialOpenAIRealtimeEndpoint(process.env.OPENAI_BASE_URL)
  ) {
    result.openai = {};
  }
  if (process.env.VOLCENGINE_REALTIME_API_KEY?.trim()) result.volc = {};
  return result;
}
