...
export interface IframePoolEntry {
  ...
  /** Whether learner pointer/keyboard interaction is allowed for this entry. */
...
export const useInteractiveIframePool = create<InteractiveIframePoolState>((set) => ({
  ...
  mount: (sceneId, input) =>
    set((state) => {
      ...
      const interactionEnabled = input.interactionEnabled !== false;
      if (existing && existing.srcDoc === input.srcDoc && existing.src === input.src) {
        const entries = {
          ...state.entries,
          [sceneId]: { ...existing, interactionEnabled, tick },
        };
        return { entries, tick };
      }
