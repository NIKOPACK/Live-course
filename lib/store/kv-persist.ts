/**
 * App wiring for zustand `persist` over the `@livecourse/storage` `KVStore`.
 *
...
class KeyState<S> {
  ...
  get needsRecovery(): boolean {
    ...
    return this.#phase !== 'settled' || this.#refused !== null || this.#replay !== null;
  }
  ...
  /** A user retry may reopen an exhausted budget, never the write gate. */
  requestRecovery(): void {
    if (!this.needsRecovery || this.#recoveryInFlight) return;
    ...
    if (this.#recoveryAttempts >= backoffMs.length) {
      ...
      log.error(
        `Giving up on recovering "${this.name}" after ${backoffMs.length} attempts; storage stays ` +
          `read-only until storage recovers or the user explicitly retries`,
      );
...
export interface KVPersistDeps {
   ...
   * cap: after this many attempts the key stays read-only until recovery or an
   * explicit user retry, rather than retrying forever.
   *
...
export interface KVPersistStorage<S> extends PersistStorage<S> {
  /**
   * Capture and await the latest `setItem` request for this key. Call directly
   * after the synchronous store update, before another update can replace it.
   * Rejects if no request exists, or that request did not reach storage. Reads
   * and recovery do not turn a failed receipt into success; retry with a new
   * write. Clearing forgets the latest receipt, without changing one already
   * captured by a caller.
   */
  waitForWrite(name: string): Promise<void>;
  /**
   * Request a fresh bounded recovery cycle after an explicit Save fails. Uses
   * the existing deferred recovery callback; healthy keys, clearing keys and
   * recovery already in flight are untouched. This neither writes directly nor
   * changes a failed receipt: the caller must still report that Save failed.
   */
...
export function createKVPersistStorage<S>(
        ...
        requestRecovery: (key, delayMs, done) => {
          setTimeout(() => {
            void Promise.resolve()
              .then(() => {
                // A clear or ordinary read may have settled the key while the
                // timer waited. Do not rehydrate an already healthy store.
                if (stateFor(key).needsRecovery) return deps.onWriteRefused?.(key);
              })
  ...
  // These are operation results, not another copy of persisted state. Keep the
  // cached promises fulfilled so passive zustand writes need no new catch.
  const writeReceipts = new Map<string, Promise<boolean>>();
  ...
  return {
    requestRecovery(name) {
      stateFor(name).requestRecovery();
    },

    async waitForWrite(name) {
      const receipt = writeReceipts.get(name);
      if (!receipt) throw new Error(`No write requested for ${JSON.stringify(name)}`);
      if (!(await receipt)) {
        throw new Error(`Could not persist ${JSON.stringify(name)} to the KV ${scope} scope`);
      }
    },
    ...
    setItem(name, value) {
      const state = stateFor(name);
      if (!state.admitWrite(value)) {
        writeReceipts.set(name, Promise.resolve(false));
        return Promise.resolve();
      }
      let saved = false;
      const written = serial(name, async () => {
        const kvStorage = resolveKvStorage();
        if (!kvStorage) {
          if (isBrowserRuntime()) {
            state.onFailure(
        ...
        const result = (await Outcome.run(() => kvStorage.setItem(name, value))).into(
          state,
          `write "${name}" to the KV ${scope} scope`,
        );
        if (result === UNAVAILABLE) state.noteWriteFailed(value);
