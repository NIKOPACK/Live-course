...
type SelectableProviderCfg = LLMProviderCfgLike & {
  name?: string;
  models: Array<{ id: string; name?: string }>;
};

/**
 * Flat model list of already-usable providers. Channel / key / base-URL /
 * model selection stays in Settings.
 */
export function listSelectableModels(
  providersConfig: Record<string, SelectableProviderCfg> | undefined | null,
): SelectableModelOption[] {
  if (!providersConfig) return [];
  const options: SelectableModelOption[] = [];
  for (const [providerId, config] of Object.entries(providersConfig)) {
    if (!isLLMProviderConfigured(config)) continue;
    const providerName = config.name?.trim() || providerId;
