 ...
 * Settings Store
 *
 * Global settings, persisted through the `@livecourse/storage` KVStore in the
...
export interface SettingsState {
  ...
  webSearchProvidersConfig: Record<
    WebSearchProviderId,
    {
      ...
      modelId?: string;
      /** Zhihu global_search index. */
      searchDB?: ZhihuSearchDB;
      /** Zhihu Filter expression (host / publish_time). */
      filter?: string;
    }
  >;
  ...
  // Full-duplex classroom voice (OpenAI Realtime / Volc). Distinct from ASR/TTS.
  realtimeProviderId: RealtimeProviderId;
  realtimeProvidersConfig: Record<RealtimeProviderId, RealtimeProviderSettings>;
  setRealtimeProvider: (providerId: RealtimeProviderId) => void;
  setRealtimeProviderConfig: (
    providerId: RealtimeProviderId,
    config: Partial<RealtimeProviderSettings>,
  ) => void;
...
// Initialize default Web Search config (Zhihu Global Search only)
const getDefaultWebSearchConfig = () => ({
  webSearchEnabled: false,
  webSearchProviderId: 'zhihu' as WebSearchProviderId,
  webSearchProvidersConfig: {
    zhihu: {
      apiKey: '',
      baseUrl: WEB_SEARCH_PROVIDERS.zhihu.defaultBaseUrl || '',
      enabled: true,
      requiresApiKey: true,
      searchDB: 'all' as ZhihuSearchDB,
      filter: '',
    },
  } as Record<
    WebSearchProviderId,
...
function getDefaultRealtimeConfig() {
  return {
    realtimeProviderId: 'openai' as RealtimeProviderId,
    realtimeProvidersConfig: {
      openai: { apiKey: '', enabled: true },
      volc: { apiKey: '', enabled: true },
    } as Record<RealtimeProviderId, RealtimeProviderSettings>,
  };
}
...
function ensureBuiltInWebSearchProviders(state: Partial<SettingsState>): void {
  ...
  // Drop entries for providers removed from the registry (e.g. tavily/bocha/
  // brave/baidu/claude/minimax/doubao/searxng persisted before the
  // Zhihu-only cleanup) so stale credentials can't linger in client state.
  for (const pid of Object.keys(state.webSearchProvidersConfig)) {
    if (!(pid in WEB_SEARCH_PROVIDERS)) {
      delete (state.webSearchProvidersConfig as Record<string, unknown>)[pid];
    }
  }
...
function ensureBuiltInRealtimeProviders(state: Partial<SettingsState>): void {
  const defaults = getDefaultRealtimeConfig();
  if (!state.realtimeProvidersConfig) {
    state.realtimeProviderId = defaults.realtimeProviderId;
    state.realtimeProvidersConfig = defaults.realtimeProvidersConfig;
    return;
  }
  for (const providerId of Object.keys(defaults.realtimeProvidersConfig) as RealtimeProviderId[]) {
    if (!state.realtimeProvidersConfig[providerId]) {
      state.realtimeProvidersConfig[providerId] = defaults.realtimeProvidersConfig[providerId];
    }
  }
  if (state.realtimeProviderId !== 'openai' && state.realtimeProviderId !== 'volc') {
    state.realtimeProviderId = defaults.realtimeProviderId;
  }
}
...
const settingsStorage = createKVPersistStorage<Partial<SettingsState>>('account', {
  ...
  persist(
    (set, get) => {
      ...
      return {
        ...
        setWebSearchProvider: (providerId) => {
          if (!(providerId in WEB_SEARCH_PROVIDERS)) return;
          set({ webSearchProviderId: providerId });
        },
        setWebSearchProviderConfig: (providerId, config) =>
          set((state) => {
            if (!(providerId in WEB_SEARCH_PROVIDERS)) return state;
        ...
        setRealtimeProvider: (providerId) => set({ realtimeProviderId: providerId }),
        setRealtimeProviderConfig: (providerId, config) =>
          set((state) => ({
            realtimeProvidersConfig: {
              ...state.realtimeProvidersConfig,
              [providerId]: {
                ...state.realtimeProvidersConfig[providerId],
                ...config,
              },
            },
          })),
        ...
        fetchServerProviders: async () => {
          try {
            ...
            const data = (await res.json()) as {
              ...
              realtime?: Record<string, Record<string, never>>;
            ...
            set((state) => {
              ...
              const newRealtimeConfig = {
                ...getDefaultRealtimeConfig().realtimeProvidersConfig,
                ...state.realtimeProvidersConfig,
              };
              for (const key of Object.keys(newRealtimeConfig) as RealtimeProviderId[]) {
              ...
              if (data.realtime) {
                for (const pid of Object.keys(data.realtime)) {
                  const key = pid as RealtimeProviderId;
              ...
              return {
                ...
                realtimeProvidersConfig: newRealtimeConfig,
    ...
    },
    {
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        // 大纲审阅已下线（docs/spec/02 生成预览节）：丢弃遗留开关。
        delete (state as Record<string, unknown>).reviewOutlineEnabled;
        ...
        // The legacy flat key was Tavily-era and is useless for Zhihu, so it
        // is dropped rather than carried over.
        if (!state.webSearchProvidersConfig) {
          const stateRecord = state as Record<string, unknown>;
          state.webSearchProviderId = 'zhihu' as WebSearchProviderId;
          state.webSearchProvidersConfig = {
            zhihu: {
              apiKey: '',
              baseUrl: WEB_SEARCH_PROVIDERS.zhihu.defaultBaseUrl || '',
        ...
        ensureBuiltInAudioProviders(state);
        ensureBuiltInWebSearchProviders(state);
        ensureBuiltInRealtimeProviders(state);
      ...
      merge: (persistedState, currentState) => {
        ...
        // Retired with the Baidu provider: web search is Zhihu-only now.
        delete persisted.baiduSubSources;
        ...
        ensureBuiltInRealtimeProviders(merged as Partial<SettingsState>);
        ensureValidProviderSelections(merged as Partial<SettingsState>);
        stripLegacyServerBaseUrl(merged as Partial<SettingsState>);
        const typedMerged = merged as Partial<SettingsState>;
        if (typedMerged.webSearchEnabled === undefined) {
          typedMerged.webSearchEnabled = false;
        }
...
/** Explicit UI submission through the same storage queue as automatic persistence. */
export async function saveSettings(update: (state: SettingsState) => void): Promise<void> {
  // Do not rehydrate here: an asynchronous read could overwrite unrelated
  // settings edited while this submission is pending. The adapter owns recovery.
  update(useSettingsStore.getState());
  // Capture immediately after the synchronous store update, before any other
  // action can issue a write. Only this request's durable result acknowledges Save.
  try {
    await settingsStorage.waitForWrite('settings-storage');
  } catch (error) {
    settingsStorage.requestRecovery('settings-storage');
    throw error;
  }
}
