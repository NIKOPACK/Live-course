...
export interface StageLoadOptions {
  /** Load persisted content for presentation without maintenance writes. */
...
/**
 * Replay/presentation temporarily renders through the canonical Zustand store
 * so existing scene consumers can keep their subscriptions.  During that
 * window the canonical values are not document values: a flush must not take
 * a snapshot of them.  The fence is a nestable lease rather than a boolean so
 * independently mounted presentation surfaces cannot release one another's
 * protection.  The owner restores its captured view before releasing the last
 * lease; the queued save then observes the restored state.
 */
export interface StagePresentationFence {
  /** Idempotently release this lease. The outermost release resumes saves. */
  release: () => void;
}

let presentationFenceDepth = 0;
const presentationFenceWaiters = new Set<() => void>();

export function isStagePresentationFenceActive(): boolean {
  return presentationFenceDepth > 0;
}

function waitForStagePresentationFence(): Promise<void> {
  if (presentationFenceDepth === 0) return Promise.resolve();
  return new Promise((resolve) => presentationFenceWaiters.add(resolve));
}

/**
 * Pause stage-document persistence while a presentation/replay surface owns
 * the canonical in-memory store. Leases are nestable and idempotent. Releasing
 * the outermost lease wakes blocked flushes and schedules the pending work;
 * callers should restore their presentation snapshot before calling release.
 */
export function acquireStagePresentationFence(): StagePresentationFence {
  presentationFenceDepth += 1;
  // A debounce callback may already be armed. Cancel it while the canonical
  // store is carrying presentation values; release() will arm it again.
  cancelScheduledSave();
  let released = false;
  return {
    release() {
      ...
      released = true;
      presentationFenceDepth -= 1;
      if (presentationFenceDepth > 0) return;

      const waiters = [...presentationFenceWaiters];
      presentationFenceWaiters.clear();
      for (const resolve of waiters) resolve();

      // Let the lease owner finish its synchronous snapshot restoration before
      // taking a new persistence snapshot. A timer remains as a second safety
      // net for callers that do not await flushStageSave themselves.
      if (pendingChanges.size > 0 && pendingStageId) {
        schedulePendingSave();
        queueMicrotask(() => {
          if (presentationFenceDepth > 0) return;
          void flushStageSave().catch(() => {
            // flushStageSave retains failed dirt and logs the root cause.
          });
        });
      }
    },
...
function markPendingChanges(stageId: string | undefined, ...changes: PendingChange[]): void {
  ...
  // Keep descriptors collected while a presentation lease is active, but do
  // not arm a timer. The lease owner restores the canonical view before the
  // outermost release resumes the scheduler, so a later snapshot cannot carry
  // replay-only values to durable storage.
  if (presentationFenceDepth === 0) schedulePendingSave();
}
...
interface StageState {
  ...
  // A1: lesson plan persisted inside the document outline snapshot
  // (`AppDocumentOutline.lessonPlan`). Null for legacy decks — the classroom
  // falls back to deriving a plan from stage + scenes at runtime.
  lessonPlan: LessonPlan | null;

  // Versioned course metadata persisted as a dedicated document row. Null for
  // legacy documents that predate course-level identity.
  ...
  setLessonPlan: (lessonPlan: LessonPlan | null) => void;
  setCoursePlan: (coursePlan: CoursePlan | null) => void;
  ...
  const result = await saveStageDataIncremental(
    ...
    [...dirtySnapshot.values()].map(({ change }) => change),
    {
      ...
      outline: {
        ...
        // `undefined` (not null) so legacy decks keep the field absent in
        // storage; the read side treats both as "derive at runtime".
        lessonPlan: snapshot.lessonPlan ?? undefined,
        generationComplete: snapshot.generationComplete,
        createdAt: Date.now(),
        updatedAt: Date.now(),
      },
      coursePlan: snapshot.coursePlan,
    },
    capturedEpoch,
  );
...
const useStageStoreBase = create<StageState>()((set, get) => ({
  ...
  addScene: (scene) => {
    ...
    // Generation adds scenes one-by-one with a null cursor. Without this
    // heal, preview → classroom keeps scenes in memory but never selects
    // one, so the board renders a blank 16:9 surface.
    const currentSceneId = get().currentSceneId;
    const shouldSwitch = !currentSceneId || currentSceneId === PENDING_SCENE_ID;
    set({
      scenes,
      generatingOutlines,
  ...
  setCoursePlan: (coursePlan) => {
    set({ coursePlan });
    // Course metadata is part of the aggregate save contract. Reuse the
    // existing outline dirty unit so old incremental callers still take the
    // atomic full-document path and no second persistence queue is needed.
    markPendingChanges(get().stage?.id, { kind: 'outline' });
  },
  ...
  saveToStorage: async () => {
    // Aggregate writes (generation completion / hydration) do not go through
    // the pending-change scheduler. Hold them behind the same presentation
    // fence and re-read the state after replay restores its snapshot; using a
    // pre-fence capture here would persist the transient presentation view.
    if (presentationFenceDepth > 0) {
      await waitForStagePresentationFence();
      return get().saveToStorage();
    }
    const {
      stage,
      scenes,
  ...
  loadFromStorage: async (
    ...
    try {
      ...
      const persistedLessonPlan = outlinesRecord?.lessonPlan ?? null;
      ...
      if (data) {
        ...
        set({
          ...
          outlines,
          lessonPlan: persistedLessonPlan,
          coursePlan: data.coursePlan ?? null,
          generationComplete,
        ...
        // A replay load may infer completion for rendering purposes, but must
        // never self-heal the canonical outline row. The teaching/editor path
        // retains the legacy compatibility write.
        if (!readOnly && generationComplete && !persistedComplete) {
          void get().saveToStorage();
        }
...
function startFlushRound(): FlushRound | null {
  if (presentationFenceDepth > 0) return null;
...
export async function flushStageSave(): Promise<void> {
  // A presentation lease owns the canonical in-memory view. Leave all pending
  // descriptors queued and let the outermost release schedule a fresh round
  // after the owner has restored its snapshot.
  if (presentationFenceDepth > 0) return;
