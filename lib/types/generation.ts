...
/** Homepage upload/cache state; never serialized into GenerationSession. */
export type CourseMaterialUpload = SelectedCourseMaterial &
  (
    | { status: 'uploading' }
    | { status: 'completed'; storageKey: string }
    | { status: 'failed'; error: string }
  ) & { removing?: boolean; removalError?: string };
...
export interface UserRequirements {
  ...
  clarificationAnswers?: ClarifyAnswer[]; // Clarifier answers narrowing scope/level (A3)
  selectedTopics?: string[]; // Learner-checked knowledge topic titles (A3)
}
