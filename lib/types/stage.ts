...
// VideoManifest / SlideContent / QuizContent / …) now lives in `@livecourse/dsl` and
...
export { isSlideContent, isQuizContent } from '@livecourse/dsl';

// `@livecourse/dsl` inlines the question-type union on `QuizQuestion.type` rather than
// exporting a named alias; derive it here so editor quiz code can keep importing
// `QuizQuestionType` from `@/lib/types/stage`.
export type QuizQuestionType = import('@livecourse/dsl').QuizQuestion['type'];
...
export type { SceneContent as SceneContentBase } from '@livecourse/dsl';
...
export type { Scene as SceneShape } from '@livecourse/dsl';
...
export type AppScene = DslScene<Action, SceneContent> & {
   ...
   * annotation only — not part of the `@livecourse/dsl` Scene contract. Absent on
