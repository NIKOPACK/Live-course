 ...
 * `{ __livecourseErrorReplayRequest: true }` message once its listener is ready. The
...
(function () {
  ...
  function emit(errorKind, message) {
    try {
      window.parent.postMessage(
        { __livecourseInteractive: true, kind: 'runtime-error', errorKind: errorKind, message: message },
        '*'
      );
    } catch (e) {}
  }
  ...
  window.addEventListener('message', function (e) {
    var d = e && e.data;
    if (d && d.__livecourseErrorReplayRequest === true) {
