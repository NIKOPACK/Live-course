...
import { DocumentVersionError } from '@livecourse/storage';
...
export interface StageStoreData {
  ...
  /** Versioned course metadata. Omission means preserve the existing row. */
...
function documentSnapshot(
  ...
  // A1：教案随大纲快照持久化（AppDocumentOutline.lessonPlan）。调用方重建
  // outline 时未必携带 lessonPlan —— 省略时保留既有教案，不静默丢掉。
  const lessonPlan = outline.lessonPlan ?? existingOutline?.lessonPlan;
  // `coursePlan` is a separate document metadata line (not part of the
  // opaque outline envelope). A caller that does not carry it must not erase
  // an already durable plan while an aggregate is rebuilt. Validate the
  // selected value at this seam too; malformed data must fail loud even when
  // a custom document adapter omits the package-level validator.
  // Keep the three states distinct at the aggregate seam:
  //   - omitted (`undefined`) means this caller did not carry metadata, so an
  //     existing row must survive a document rebuild;
  //   - explicit `null` means the caller intentionally reset the plan;
  //   - a concrete value replaces the row after validation.
  // Using `??` here would collapse the latter two and make an explicit reset
  // silently preserve stale course identity.
  const candidateCoursePlan = data.coursePlan !== undefined ? data.coursePlan : existingCoursePlan;
  const coursePlan = candidateCoursePlan == null ? undefined : parseCoursePlan(candidateCoursePlan);
  return {
    ...
    await mutateDocument(
      ...
      async (existing, store) => {
        ...
        await withRuntimeStorageSharedLock(async () => {
          ...
          const existingCoursePlan = existing?.coursePlan;
          if (isStageWriteStale(stageId, capturedEpoch)) {
            dropped = true;
            return;
          }
          await store.saveDocument(
            documentSnapshot(stageId, data, existingOutline, existingCoursePlan, now),
          );
          if (isStageWriteStale(stageId, capturedEpoch)) {
            dropped = true;
            return;
...
export async function loadStageData(stageId: string): Promise<StageStoreData | null> {
  try {
    ...
    return {
      ...
      chats,
      chatSnapshot,
      // Keep absence explicit for the Zustand state while preserving the
      // document-store distinction between an omitted metadata row and a
      // concrete CoursePlan value.
      coursePlan: document.coursePlan === undefined ? null : parseCoursePlan(document.coursePlan),
...
type ThumbnailSlide = import('@livecourse/dsl').Slide;
            ...
            for (const el of mediaElements) {
              const videoBinding =
                el.type === 'video'
                  ? resolveVideoMediaForElement(
                      taskEntries,
                      el as import('@livecourse/dsl').PPTVideoElement,
                      stageId,
                      documentElements,
                    )
                  : undefined;
              ...
              const selected =
                el.type === 'video'
                  ...
                  : resolveMediaTaskForElement(
                      taskEntries,
                      el as import('@livecourse/dsl').PPTElement,
                      stageId,
                    );
