...
export interface EmitHyperframesOptions {
  ...
  height?: number;
  /** Composition id used for the root `data-composition-id` and the timeline key. Default `livecourse`. */
  compositionId?: string;
  /** Relative path the emitted HTML loads GSAP from. Default `assets/vendor/gsap.min.js`. */
  gsapVendorPath?: string;
  /** Manifest filename. Default `livecourse-video-manifest.json`. */
  manifestPath?: string;
...
const DEFAULT_MANIFEST = 'livecourse-video-manifest.json';
...
function renderReadme(project: {
  ...
  return `# ${project.stageName} — LiveCourse video export
...
export function emitHyperframes(
  ...
  const compositionId = options.compositionId ?? 'livecourse';
...
<title>${escapeHtml(ir.stage.name)} — LiveCourse video</title>
<style>
  ${INTER_FONT_FACE_CSS}
