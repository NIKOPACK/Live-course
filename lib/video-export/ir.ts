 ...
 *
 * Pure: depends only on `@livecourse/dsl` (the `SceneType` set) and `zod`.
 */
import { z } from 'zod';
import { SCENE_TYPES } from '@livecourse/dsl';

/** Manifest `schema` tag — stable across versions; the shape is versioned by {@link VIDEO_TIMELINE_VERSION}. */
export const VIDEO_TIMELINE_SCHEMA = 'livecourse.videoTimeline';
...
export const VIDEO_TIMELINE_COMPILER = 'livecourse-video-timeline';
