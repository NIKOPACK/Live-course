...
The **contract keystone** of the LiveCourse SDK family. `@livecourse/dsl` is *pure spec* — the
...
package can depend on `@livecourse/dsl` without pulling in junk.

## Dependency arrows (acyclic)

```
@livecourse/dsl       ->  (nothing)
@livecourse/renderer  ->  @livecourse/dsl
@livecourse/importer  ->  @livecourse/dsl
@livecourse/exporter  ->  @livecourse/dsl     (reserved, future)
```

`@livecourse/dsl` is the only package everything else depends on, and it depends on
nothing.
...
```ts
import type { Slide, PPTElement, Action } from '@livecourse/dsl';
import { isTextElement, DSL_VERSION, SYNC_ACTIONS } from '@livecourse/dsl';
```
...
1. **JSON Schema artifacts (cross-language mirror)** — `Stage`, the default
   ...
   ```ts
   import stageSchema from '@livecourse/dsl/schema/stage.schema.json' with { type: 'json' };
   import sceneSchema from '@livecourse/dsl/schema/scene.schema.json' with { type: 'json' };
   import actionSchema from '@livecourse/dsl/schema/action.schema.json' with { type: 'json' };
...
2. **Pure validators (in-process boundary)** — `validate*` are hand-written,
   ...
   ```ts
   import { validateStage, validateScene, validateAction } from '@livecourse/dsl';
...
3. **Pure normalizers (repair boundary)** — where `validate*` *reports* on a
   ...
   ```ts
   import { normalizeElement, normalizeScene, ELEMENT_DEFAULTS } from '@livecourse/dsl';
...
```ts
import { DSL_VERSION, migrate, dslVersionOf, needsMigration } from '@livecourse/dsl';
...
outside the document, per learner, through a `RuntimeStore` (`@livecourse/storage`).
...
Both consumers are now wired to `@livecourse/dsl` and no longer vendor their own copy
of the slide types:

- **`@livecourse/importer`**: imports all slide types from `@livecourse/dsl`; vendored
  `livecourse/types/slides.ts` deleted. The importer emits complete DSL `Slide`
  objects directly (the old partial "draft slide" + post-fill step is gone).
- **`@livecourse/renderer`**: imports all slide types from `@livecourse/dsl`; vendored
  `types/slides.ts` deleted. `@livecourse/dsl` is a regular dependency, kept external
  in the rollup build so consumers share one copy. The public
  `@livecourse/renderer/types` surface now re-exports the DSL types.

### Roadmap

- [x] Wire `@livecourse/importer` to import types from `@livecourse/dsl` (vendored copy deleted).
- [x] Wire `@livecourse/renderer` to import types from `@livecourse/dsl` (vendored copy deleted).
...
- [ ] Reserve `@livecourse/exporter` as the 4th family member.
...
```ts
import type { Action, InteractiveContent, PBLContent, Scene, SceneContent, WidgetConfigBase } from '@livecourse/dsl';
...
```bash
pnpm --filter @livecourse/dsl build         # -> dist/ (index.js, index.d.ts, …) + dist/schema/*.json
pnpm --filter @livecourse/dsl build:schema  # regenerate only dist/schema/*.json
pnpm --filter @livecourse/dsl typecheck
pnpm --filter @livecourse/dsl test
```

## License

MIT, matching the rest of the family (`@livecourse/dsl`, `@livecourse/importer`,
`@livecourse/renderer`) and the LiveCourse root, so the license policy is uniform
across the SDK.
