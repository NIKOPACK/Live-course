 ...
 * document to another runtime. `@livecourse/renderer` already consumes resolved
 * URLs via its media slots; `@livecourse/exporter` will consume the set of refs a
 ...
 * CDN on a server — live in `@livecourse/storage`, keeping this package
 ...
 * pure package to the DOM lib. Backends in `@livecourse/storage` accept a real
