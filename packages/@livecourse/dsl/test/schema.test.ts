...
import Ajv from 'ajv';
import { validateAction, validateScene } from '@livecourse/dsl';
...
describe('generated JSON Schema — SerializedScene', () => {
  ...
  it('accepts optional model HTML while retaining required structured quiz questions', () => {
    const scene = {
      id: 'html-quiz',
      stageId: 'stage',
      type: 'quiz',
      title: 'Quiz',
      order: 0,
      content: { type: 'quiz', questions: [], html: '<html><head></head><body>Quiz</body></html>' },
    };
    expect(v(scene)).toBe(true);
    expect(validateScene(scene)).toEqual({ valid: true });
    expect(v({ ...scene, content: { type: 'quiz', questions: [] } })).toBe(true);
    expect(v({ ...scene, content: { type: 'quiz', html: scene.content.html } })).toBe(false);
  });
  const slideScene = {
    id: 'sc',
    stageId: 'st',
