...
/**
 * DeepSeek often closes a JSON array and then emits an extra quote before the
 * next object key, sometimes with a stray `.` or `,` in between:
 * `"]."explanationPlan":` / `"]","explanationPlan":` / `]"explanationPlan":`
 */
function repairStrayQuoteAfterArray(jsonStr: string): string {
  return jsonStr
...
export function tryParseJson<T>(jsonStr: string, options: JsonParsingOptions = {}): T | null {
  ...
  try {
    ...
    // Fix -1: Restore object keys after a closed array. Do this before any
    // quote-scanning repair so the extra quote cannot shift string boundaries.
    fixed = repairStrayQuoteAfterArray(fixed);
  ...
  try {
    const repaired = jsonrepair(repairStrayQuoteAfterArray(jsonStr));
