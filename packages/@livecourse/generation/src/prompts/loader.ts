...
const PROMPT_VARIABLE_DEFAULTS = {
  ...
  // A3 outline workflow injections render empty when clarification/scope
  // selection didn't run, so the placeholders never leak to the model.
  'requirements-to-outlines': {
    clarificationContext: '',
    selectedTopicsText: '',
  },
  // A1 lesson design injection renders empty when the lesson has no persisted
  // plan (legacy courses), so the placeholder never leaks to the model.
  'slide-content': {
    lessonDesign: '',
  },
