/**
 * High-level entry: parsed pptxtojson(-pro) JSON → LiveCourse canvas `Slide[]`.
 *
 ...
 * whole import either.
 */
import { normalizeSlideWith, type Slide } from '@livecourse/dsl';
...
/**
 * Convert a pre-parsed pptxtojson(-pro) `Output` JSON into LiveCourse slides.
 *
...
const normalizeImportedSlide = normalizeSlideWith({
  ...
  onDropped: (_element, error) => {
    console.warn(
      `[@livecourse/importer] dropping element that fails DSL normalization: ${
...
/**
 * Parse a .pptx file and convert it into LiveCourse canvas slides.
 *
