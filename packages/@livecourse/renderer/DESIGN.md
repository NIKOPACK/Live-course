# livecourse-renderer 设计稿（v1：只读画布）

> 配套计划：`docs/superpowers/plans/2026-05-28-livecourse-renderer-package.md`
> 原始 spec：`docs/superpowers/specs/2026-05-28-livecourse-renderer-package-design.md`

## 1. 目标

把 LiveCourse 主仓 `components/livecourse-renderer/` 中的只读画布部分抽成独立 workspace 包，让任意 React + Tailwind 4 项目都能"装包 → 传 Slide → 直接渲染"。
...
| D4 | 特效层 | **包含**，4 个全做成可选 props，默认关 | 还原"LiveCourse 同款画布"必备，但默认零负担 |
...
| D8 | 包发布 | workspace 包，未来可发 npm；当前名 `livecourse-renderer`（同 `pptxtojson-pro` 风格） | 一致性 |
...
```ts
import { SlideCanvas, type SlideCanvasProps } from 'livecourse-renderer';
...
```tsx
import { SlideRendererProvider, useSlideContext } from 'livecourse-renderer';
...
} from 'livecourse-renderer/elements';
```
...
} from 'livecourse-renderer/types';
```
...
1. `pnpm --filter livecourse-renderer build` 通过，产出干净 dist
2. 主仓 `/livecourse-renderer-demo` 路由用包渲染一份手写 Slide，目视与原 `ScreenCanvas` 一致
3. v1 **不替换主仓既有调用**
...
| 主仓 `Slide` 类型未来演进，包内拷贝版会漂移 | 低 | 后续可通过 `pptxtojson-pro` 已有的 `livecourse/types/slides.ts` 三方共享 |
