...
React component for rendering PPTist-style `Slide` JSON. Extracted from [LiveCourse](https://github.com/NIKOPACK/livecourse).
...
```bash
pnpm add @livecourse/renderer
# or
npm install @livecourse/renderer
```
...
```tsx
import { SlideCanvas, type Slide } from '@livecourse/renderer';
...
```tsx
import { SlideRendererProvider, SlideCanvas, useSlideContext } from '@livecourse/renderer';
...
### Granular components — `@livecourse/renderer/elements`
...
### Types — `@livecourse/renderer/types`
...
export default {
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@livecourse/renderer/dist/**/*.{js,cjs}',
  ],
};
```
...
[`@livecourse/importer`](../importer) converts `.pptx` files to the same `Slide[]` shape, so you can do `.pptx → @livecourse/renderer` end-to-end.
