...
The LiveCourse pluggable persistence layer: small, swappable-backend primitives for
persisting app state, depending only on [`@livecourse/dsl`](../dsl).
...
```
@livecourse/storage -> @livecourse/dsl
```
...
| `StorageProvider` (from `@livecourse/dsl`) | the asset seam: `put(blob) → ref`, `resolve(ref) → url`, `remove(ref)` | `BrowserAssetStore` over IndexedDB (`assets` registry + `blobs`) + object URLs |
...
data lives in the new `livecourse-asset-pool` database.
...
- `BrowserAssetStore` deliberately does not read data written by the 0.1.x
  provider in `livecourse-assets`. Its content-addressed `sha256-` refs are no longer
...
- [x] `StorageProvider` (in `@livecourse/dsl`) + browser asset registry: allocated
