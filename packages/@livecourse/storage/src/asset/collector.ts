...
function collectorFailure(): Error {
  return new Error('@livecourse/storage: asset collection failed');
}
...
export class AssetCollector {
  ...
  constructor(
    ...
    if (typeof options?.withTransaction !== 'function') {
      throw new Error('@livecourse/storage: withTransaction is required for AssetCollector');
    }
    const graceMs = options.graceMs ?? DEFAULT_ASSET_COLLECTION_GRACE_MS;
    if (!Number.isSafeInteger(graceMs) || graceMs < 0) {
      throw new Error('@livecourse/storage: graceMs must be a non-negative safe integer');
    }
