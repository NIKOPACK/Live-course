import type { AssetMeta, AssetRef, BinaryBlob, StorageProvider } from '@livecourse/dsl';
...
function assertMetadata(meta: AssetMeta): void {
  if (typeof meta !== 'object' || meta === null || Array.isArray(meta)) {
    throw new HttpAssetStoreError(
      0,
      'VALIDATION_FAILED',
      '@livecourse/storage: asset metadata must be a plain JSON object',
    );
  }
  if ('principal' in meta || 'contentHash' in meta) {
    throw new HttpAssetStoreError(
      0,
      'VALIDATION_FAILED',
      '@livecourse/storage: asset metadata contains a prohibited member',
    );
  }
  try {
    ...
    throw new HttpAssetStoreError(
      0,
      'VALIDATION_FAILED',
      error instanceof Error ? error.message : '@livecourse/storage: invalid asset metadata',
    );
  }
}
...
export class HttpAssetStore implements StorageProvider {
  ...
  constructor(options: HttpAssetStoreOptions) {
    ...
    if (typeof selectedFetch !== 'function') {
      throw new Error('@livecourse/storage: HttpAssetStore requires a fetch implementation');
    }
  ...
  private async headers(
    ...
    try {
      ...
      throw new HttpAssetStoreError(
        0,
        'HTTP_REQUEST_FAILED',
        '@livecourse/storage: asset request headers could not be constructed',
      );
    }
    if (multipart && headers['content-type'] !== undefined) {
      throw new HttpAssetStoreError(
        ...
        '@livecourse/storage: the headers hook must not set Content-Type for multipart asset writes',
      );
    }
    return headers;
  }
  ...
  private async fetchResponse(method: string, path: string, body?: Blob): Promise<Response> {
    if (this.closed) {
      throw new HttpAssetStoreError(
        0,
        'STORE_CLOSED',
        '@livecourse/storage: HttpAssetStore is closed',
      );
    }
    ...
    try {
      ...
      throw new HttpAssetStoreError(
        0,
        'HTTP_REQUEST_FAILED',
        '@livecourse/storage: asset HTTP request failed',
      );
    }
  }
  ...
  private async httpError(response: Response): Promise<HttpAssetStoreError> {
    ...
    const message =
      typeof body?.error?.message === 'string'
        ? body.error.message
        : `@livecourse/storage: asset HTTP request failed with status ${response.status}`;
  ...
  private async writeForm(
    ...
    try {
      ...
      throw new HttpAssetStoreError(
        0,
        'VALIDATION_FAILED',
        '@livecourse/storage: asset bytes could not be read',
      );
    }
    if (!/^[\x20-\x7e]*$/.test(data.type)) {
      throw new HttpAssetStoreError(
        0,
        'VALIDATION_FAILED',
        '@livecourse/storage: asset media type contains characters that cannot be carried in a header',
      );
    }
    ...
    if (includeMeta) {
      try {
        ...
        throw new HttpAssetStoreError(
          0,
          'VALIDATION_FAILED',
          '@livecourse/storage: asset metadata could not be serialized',
        );
      }
    }
    let boundary: string;
    try {
      ...
      throw new HttpAssetStoreError(
        0,
        'HTTP_REQUEST_FAILED',
        '@livecourse/storage: a safe multipart boundary could not be generated',
      );
    }
  ...
  async put(data: BinaryBlob, meta?: AssetMeta): Promise<AssetRef> {
    ...
    try {
      ...
      throw malformed(
        response.status,
        '@livecourse/storage: asset allocation response was not valid JSON',
      );
    }
    const id =
    ...
    if (
      ...
      throw malformed(
        response.status,
        '@livecourse/storage: asset allocation response must carry an id and revision',
      );
    }
    return id;
  }
  ...
  private async get(
    ...
    if (response.status !== 200 || identity === null) {
      throw malformed(
        response.status,
        '@livecourse/storage: asset byte response must carry content type and revision',
      );
    }
    let bytes: ArrayBuffer;
    try {
      ...
      throw malformed(
        response.status,
        '@livecourse/storage: asset byte response could not be read',
      );
    }
    ...
    const url = await this.urls.resolve(id, identity, async () => {
      ...
      try {
        ...
        throw malformed(
          response.status,
          '@livecourse/storage: asset object URL could not be created',
        );
      }
  ...
  async remove(id: AssetRef): Promise<void> {
    ...
    if (!response.ok) throw await this.httpError(response);
    if (response.status !== 204) {
      throw malformed(response.status, '@livecourse/storage: asset removal response must be 204');
    }
    await this.invalidate(id);
  }
  ...
  async replace(id: AssetId, data: BinaryBlob, meta?: AssetMeta): Promise<void> {
    ...
    if (response.status !== 204 || revision === null || revision === '') {
      throw malformed(
        response.status,
        '@livecourse/storage: asset replacement response must be 204 with a revision',
      );
    }
    await this.invalidate(id);
  }
