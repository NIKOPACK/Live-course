...
import type { Scene, Stage } from '@livecourse/dsl';
import type {
  DocumentStore,
  DocumentSummary,
  LiveCourseDocument,
...
function assertValid(result: ReturnType<typeof validateStage>, label: string): void {
  ...
  throw new Error(`@livecourse/storage: invalid ${label}: ${detail}`);
}
...
function assertStorableScene(scene: SceneLike, stageId: string): void {
  ...
  if (typeof s.id !== 'string') {
    throw new Error(`@livecourse/storage: scene id must be a string, got ${JSON.stringify(s.id)}`);
  }
  if (s.stageId !== stageId) {
    throw new Error(
      `@livecourse/storage: scene ${JSON.stringify(s.id)} has stageId ` +
        `${JSON.stringify(s.stageId)} but belongs to document ${JSON.stringify(stageId)}`,
    );
  }
  if (typeof s.order !== 'number' || !Number.isFinite(s.order)) {
    throw new Error(
      `@livecourse/storage: scene ${JSON.stringify(s.id)} order must be a finite number, got ` +
...
function migrateDocument<TScene extends SceneLike, TStage extends Stage>(
  ...
  const { outline, coursePlan, ...core } = doc;
  const migrated = migrate(core) as LiveCourseDocument<TScene, TStage>;
  const result = outline === undefined ? migrated : { ...migrated, outline };
  if (coursePlan !== undefined) result.coursePlan = coursePlan;
  return result;
}
...
> implements DocumentStore<TScene, TStage> {
  ...
  constructor(options: BrowserDocumentStoreOptions = {}) {
    ...
    this.dbName = options.dbName ?? 'livecourse-documents';
  ...
  private openDb(): Promise<IDBDatabase> {
    ...
    this.dbPromise ??= new Promise<IDBDatabase>((resolve, reject) => {
      const req = this.idb.open(this.dbName, 2);
      req.onupgradeneeded = () => {
        const db = req.result;
        ...
        if (!db.objectStoreNames.contains(COURSE_PLANS)) {
          db.createObjectStore(COURSE_PLANS, { keyPath: 'stageId' });
        }
      };
  ...
  async saveDocument(doc: LiveCourseDocument<TScene, TStage>): Promise<void> {
    ...
    if (isFutureVersioned(doc)) {
      throw new DocumentVersionError(
        ...
        'future',
        doc.dslVersion,
        `@livecourse/storage: refusing to save document ${JSON.stringify(doc.stage.id)} — it was ` +
    ...
    if (normalized.coursePlan !== undefined) {
      assertWritableCoursePlan(normalized.coursePlan, normalized.stage.id);
    }
    ...
    for (const scene of normalized.scenes) {
      ...
      if (seen.has(scene.id)) {
        throw new Error(
          `@livecourse/storage: duplicate scene id ${JSON.stringify(scene.id)} in document ${JSON.stringify(stageId)}`,
        );
      }
      seen.add(scene.id);
    }

    const { stageRow, sceneRows, outlineRow, coursePlanRow } = splitDocument(normalized);

    await this.txRun([STAGES, SCENES, OUTLINES, COURSE_PLANS], 'readwrite', async (tx) => {
      ...
      if (existingStage && isFutureVersioned(existingStage)) {
        throw new DocumentVersionError(
          ...
          `@livecourse/storage: refusing to overwrite document ${JSON.stringify(stageId)} — the ` +
      ...
      const coursePlans = tx.objectStore(COURSE_PLANS);
      if (coursePlanRow) coursePlans.put(coursePlanRow);
      else coursePlans.delete(stageId);
    });
  }

  async loadDocument(stageId: string): Promise<LiveCourseDocument<TScene, TStage> | null> {
    const rows = await this.txRun(
      [STAGES, SCENES, OUTLINES, COURSE_PLANS],
      'readonly',
      async (tx) => {
        ...
        const outlineRow = await reqP<OutlineRow | undefined>(
          tx.objectStore(OUTLINES).get(stageId),
        );
        const coursePlanRow = await reqP<CoursePlanRow | undefined>(
          tx.objectStore(COURSE_PLANS).get(stageId),
        );
        return { stageRow, sceneRows, outlineRow, coursePlanRow };
      },
    );
    if (!rows) return null;

    const document = reassembleDocument(
      rows.stageRow,
      rows.sceneRows,
      rows.outlineRow,
      rows.coursePlanRow,
    );
    if (document.coursePlan !== undefined) {
      document.coursePlan = migrateCoursePlan(document.coursePlan, stageId);
    }
    // Migrate the aggregate forward (outline and course-plan metadata are
    // independent compatibility lines); future records are never downgraded.
  ...
  async deleteDocument(stageId: string): Promise<void> {
    ...
    await this.txRun([STAGES, SCENES, OUTLINES, COURSE_PLANS], 'readwrite', async (tx) => {
      ...
      tx.objectStore(COURSE_PLANS).delete(stageId);
      const scenes = tx.objectStore(SCENES);
  ...
  async putStage(stageId: string, stage: TStage): Promise<void> {
    ...
    if (stage.id !== stageId) {
      throw new Error(
        `@livecourse/storage: stage ${JSON.stringify(stage.id)} does not belong to document ` +
    ...
    await this.txRun([STAGES], 'readwrite', async (tx) => {
      ...
      if (!stageRow) {
        throw new DocumentNotFoundError(
          stageId,
          `@livecourse/storage: cannot putStage into missing document ${JSON.stringify(stageId)}`,
        );
      }
      if (dslVersionOf(stageRow) !== DSL_VERSION) {
        throw new DocumentVersionError(
          ...
          `@livecourse/storage: cannot putStage into document ${JSON.stringify(stageId)} at DSL ` +
  ...
  async putScene(stageId: string, scene: TScene): Promise<void> {
    ...
    await this.txRun([STAGES, SCENES], 'readwrite', async (tx) => {
      ...
      if (!stageRow) {
        throw new DocumentNotFoundError(
          stageId,
          `@livecourse/storage: cannot putScene into missing document ${JSON.stringify(stageId)}`,
        );
      }
      ...
      if (dslVersionOf(stageRow) !== DSL_VERSION) {
        throw new DocumentVersionError(
          ...
          'not-current',
          stageRow[DSL_VERSION_KEY],
          `@livecourse/storage: cannot putScene into document ${JSON.stringify(stageId)} at DSL ` +
            `version ${JSON.stringify(dslVersionOf(stageRow))} — load and save it to bring it ` +
            `to ${DSL_VERSION} first`,
        );
      }
    ...
    await this.txRun([STAGES, SCENES], 'readwrite', async (tx) => {
      ...
      if (dslVersionOf(stageRow) !== DSL_VERSION) {
        throw new DocumentVersionError(
          ...
          'not-current',
          stageRow[DSL_VERSION_KEY],
          `@livecourse/storage: cannot deleteScene from document ${JSON.stringify(stageId)} at DSL ` +
            `version ${JSON.stringify(dslVersionOf(stageRow))} — load and save it to bring it ` +
            `to ${DSL_VERSION} first`,
        );
      }
