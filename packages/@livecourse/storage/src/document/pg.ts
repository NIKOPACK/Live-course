...
function encodeJson(value: unknown, label: string): string {
  try {
    ...
    throw new Error(`@livecourse/storage: ${label} is not JSON-serializable`, { cause: error });
  }
}
...
function assertStorableScene(scene: SceneLike, stageId: string): void {
  ...
  if (typeof candidate.id !== 'string') {
    throw new Error(
      `@livecourse/storage: scene id must be a string, got ${JSON.stringify(candidate.id)}`,
    );
  }
  if (candidate.stageId !== stageId) {
    throw new Error(
      `@livecourse/storage: scene ${JSON.stringify(candidate.id)} has stageId ` +
  ...
  if (typeof candidate.order !== 'number' || !Number.isFinite(candidate.order)) {
    throw new Error(
      `@livecourse/storage: scene ${JSON.stringify(candidate.id)} order must be a finite number, ` +
...
> implements DocumentStore<TScene, TStage> {
  ...
  constructor(queryable: Queryable, options: PgDocumentStoreOptions) {
    if (typeof options?.withTransaction !== 'function') {
      throw new Error(
        '@livecourse/storage: withTransaction is required and must pin a fresh connection and ' +
  ...
  private async loadStage(
    ...
    if (!isPlainObject(decoded)) {
      throw new Error(
        `@livecourse/storage: corrupt stored row for document ${JSON.stringify(stageId)}: ` +
  ...
  > {
    ...
    const coursePlan = await queryable.query<StoredJsonRow>(
      `SELECT data
         FROM document_course_plans
        WHERE stage_id = $1`,
      [stageId],
    );
    const outlineRow = outline.rows[0]
      ...
      : undefined;
    const coursePlanRow: CoursePlanRow | undefined = coursePlan.rows[0]
      ? { stageId, coursePlan: decodeJson<unknown>(coursePlan.rows[0].data) }
      : undefined;
  ...
  private currentVersionError(
    ...
    return new DocumentVersionError(
      ...
      `@livecourse/storage: cannot ${operation} document ${JSON.stringify(stageId)} at DSL ` +
  ...
  private validateForSave(
    ...
    for (const scene of doc.scenes) {
      ...
      if (seen.has(scene.id)) {
        throw new Error(
          `@livecourse/storage: duplicate scene id ${JSON.stringify(scene.id)} in document ` +
    ...
    if (rows.coursePlanRow) {
      assertJsonValue(
        rows.coursePlanRow.coursePlan,
        `document course plan ${JSON.stringify(stageId)}`,
      );
    }
    return rows;
  }
  ...
  async saveDocument(doc: LiveCourseDocument<TScene, TStage>): Promise<void> {
    ...
    const { stageRow, sceneRows, outlineRow, coursePlanRow } = this.validateForSave(normalized);
    ...
    await this.transaction(async (queryable) => {
      ...
      if (coursePlanRow) {
        await queryable.query(
          `INSERT INTO document_course_plans (stage_id, data)
           VALUES ($1, $2::jsonb)
           ON CONFLICT (stage_id) DO UPDATE SET data = EXCLUDED.data`,
          [
            stageId,
            encodeJson(coursePlanRow.coursePlan, `document course plan ${JSON.stringify(stageId)}`),
          ],
        );
      } else {
        await queryable.query('DELETE FROM document_course_plans WHERE stage_id = $1', [stageId]);
      }
    });
  }
