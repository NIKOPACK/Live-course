 ...
 */
import type { Stage, Scene } from '@livecourse/dsl';
 ...
 *
 * `coursePlan` is optional versioned course metadata. It is stored in its own
 * row/table and migrated at the storage boundary, independently of the DSL
 * migration ladder. Unsupported or unversioned writes fail explicitly.
 *
 ...
 */
export interface LiveCourseDocument<
...
> {
  ...
  scenes: TScene[];
  outline?: unknown;
  /** Optional versioned course-level metadata, migrated independently of the DSL. */
  coursePlan?: unknown;
  dslVersion?: string;
}
...
export interface DocumentStore<TScene extends SceneLike = Scene, TStage extends Stage = Stage> {
   ...
   * {@link putScene}.)
   */
  saveDocument(doc: LiveCourseDocument<TScene, TStage>): Promise<void>;
   ...
   */
  loadDocument(stageId: string): Promise<LiveCourseDocument<TScene, TStage> | null>;
