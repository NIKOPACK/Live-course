...
export interface BrowserKVStoreOptions {
   ...
   * `localStorage` keys the app writes. Defaults to `livecourse`.
   */
  namespace?: string;
}
...
export class BrowserKVStore implements LocalKVStore {
  ...
  constructor(options: BrowserKVStoreOptions = {}) {
    ...
    this.namespace = options.namespace ?? 'livecourse';
  }
  ...
  async set<T>(key: string, value: T, scope: KVScope = DEFAULT_KV_SCOPE): Promise<void> {
    ...
    if (json === undefined) {
      ...
      throw new Error(
        `@livecourse/storage: kv value for key ${JSON.stringify(key)} serialized to undefined ` +
