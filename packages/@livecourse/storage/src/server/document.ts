...
import type { Scene, Stage, ValidationResult } from '@livecourse/dsl';
...
import {
  ...
  assertWritableCoursePlan,
  migrateCoursePlan,
} from '../document/course-plan.js';
...
function validationError(result: ValidationResult, label: string): void {
  ...
  throw validationFailure(`@livecourse/storage: invalid ${label}: ${detail}`, details);
}

function assertAddressableSegment(value: string): void {
  if (value === '.' || value === '..') {
    throw validationFailure(
      `@livecourse/storage: URL path segment must not be ${JSON.stringify(value)}`,
    );
  }
}
...
function missingDocument(stageId: string): DocumentHttpError {
  return new DocumentHttpError(
    404,
    'DOCUMENT_NOT_FOUND',
    `@livecourse/storage: missing document ${JSON.stringify(stageId)}`,
  );
}
...
function missingScene(stageId: string, sceneId: string): DocumentHttpError {
  return new DocumentHttpError(
    404,
    'SCENE_NOT_FOUND',
    `@livecourse/storage: no scene ${JSON.stringify(sceneId)} in document ${JSON.stringify(stageId)}`,
  );
}
...
function migrateDocument<TScene extends SceneLike, TStage extends Stage>(
  document: LiveCourseDocument<TScene, TStage>,
): LiveCourseDocument<TScene, TStage> {
  const { outline, coursePlan, ...core } = document;
  let migrated: LiveCourseDocument<TScene, TStage>;
  try {
    migrated = migrate(core) as LiveCourseDocument<TScene, TStage>;
  } catch (error) {
    throw validationFailure(error instanceof Error ? error.message : String(error));
  }
  const result = outline === undefined ? migrated : { ...migrated, outline };
  if (coursePlan !== undefined)
    result.coursePlan = migrateCoursePlan(coursePlan, document.stage.id);
  return result;
}
...
function assertStorableScene(scene: unknown, stageId: string): asserts scene is SceneLike {
  ...
  if (typeof value.id !== 'string') {
    throw validationFailure(
      `@livecourse/storage: scene id must be a string, got ${JSON.stringify(value.id)}`,
    );
  }
  if (value.stageId !== stageId) {
    throw validationFailure(
      `@livecourse/storage: scene ${JSON.stringify(value.id)} has stageId ` +
        `${JSON.stringify(value.stageId)} but belongs to document ${JSON.stringify(stageId)}`,
    );
  }
  if (typeof value.order !== 'number' || !Number.isFinite(value.order)) {
    throw validationFailure(
      `@livecourse/storage: scene ${JSON.stringify(value.id)} order must be a finite number, got ` +
...
function validateDocument<TScene extends SceneLike, TStage extends Stage>(
  ...
  const document = body as unknown as LiveCourseDocument<TScene, TStage>;
  if (isFutureVersioned(document)) {
    throw futureVersion(
      `@livecourse/storage: refusing to save document ${JSON.stringify(pathStageId)} — it was ` +
  ...
  if (document.coursePlan !== undefined) {
    try {
      assertWritableCoursePlan(document.coursePlan, pathStageId);
    } catch (error) {
      if (error instanceof CoursePlanMigrationError) {
        throw new DocumentHttpError(400, 'COURSE_PLAN_MIGRATION_FAILED', error.message, {
  ...
  if (normalized.stage.id !== pathStageId) {
    throw validationFailure(
      `@livecourse/storage: stage ${JSON.stringify(normalized.stage.id)} does not belong to ` +
...
async function route<TScene extends SceneLike, TStage extends Stage>(
  ...
  if (!(await (options.authorizeDocuments?.(principal, req) ?? true))) {
    throw new DocumentHttpError(
      403,
      'FORBIDDEN_DOCUMENTS',
      '@livecourse/storage: document authorization required',
    );
  }
...
>(store: DocumentStore<TScene, TStage>, options: DocumentHttpHandlerOptions): RequestListener {
  if (typeof options?.authenticate !== 'function') {
    throw new Error('@livecourse/storage: createDocumentHttpHandler requires authenticate');
  }
  ...
  return (req, res) => {
    void route(req, res, store, options).catch((error: unknown) => {
      ...
      if (!(error instanceof DocumentHttpError) || error.status >= 500) {
        console.error('@livecourse/storage: Document HTTP handler internal error', error);
      }
