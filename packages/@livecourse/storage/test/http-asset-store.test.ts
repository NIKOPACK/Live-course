      ...
      if (kind === 'own') {
        ...
        expect(JSON.parse(get.body.toString())).toEqual({
          error: {
            ...
            message: '@livecourse/storage: no asset is stored under that id',
          },
        });
        const contentLength = String(get.body.byteLength);
        expect(getHeaders).toEqual({
          'access-control-expose-headers': 'X-Asset-Revision, X-Error-Code',
          connection: 'keep-alive',
  ...
  test.each([
    ...
    expect(JSON.parse(response.body.toString())).toEqual({
      error: {
        ...
        message: '@livecourse/storage: malformed multipart body',
      },
    });
  });
