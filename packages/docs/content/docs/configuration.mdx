...
LiveCourse reads environment variables when the server starts. Every setting is optional—enable only what you need. See [`.env.example`](https://github.com/NIKOPACK/livecourse/blob/main/.env.example) in the repository for environment variable examples. See [Supported models](./supported-models.mdx) for built-in model IDs.
...
The exact course-material formats depend on the selected parser. LiveCourse currently supports text, PDF, Office documents, images, and some audio/video formats; supported formats and capabilities vary by provider.
...
`unpdf` is built into LiveCourse and supports basic PDF parsing. For Office documents, images, and material requiring OCR, table, formula, or layout analysis, select a compatible MinerU or AliDocMind provider.
...
The frontend lets users turn search on for lesson generation. Search is off by default. Optional Pi Director web search through an OpenAI-compatible Responses API is configured separately (`LIVECOURSE_ENABLE_PI_WEB_SEARCH`, `RESPONSES_WEB_SEARCH_*`). Grok web search is provided through the Grok LLM search tool, not a standalone web-search provider.
...
Visitors are prompted only once; the password is stored in an HTTP-only cookie. Leave it empty to disable access control.
...
Use `MODEL_ROUTES` to choose models for individual generation stages. Unconfigured stages continue to resolve through the client model and `DEFAULT_MODEL`. Its value is a JSON object whose keys are generation stages and whose values can be model strings or objects containing `model` and `thinking`. See [`.env.example`](https://github.com/NIKOPACK/livecourse/blob/main/.env.example) for the complete stage list and examples.
...
```bash
# LiveCourse Editor (Pro mode)
NEXT_PUBLIC_LIVECOURSE_EDITOR_ENABLED=true

# Pi chat runtime
NEXT_PUBLIC_PI_CHAT_ENABLED=true

# Vocational education task engine (server-side flag)
LIVECOURSE_ENABLE_VOCATIONAL=true
...
Environment variables override matching YAML provider fields one by one. Keys must use provider IDs registered in the code, such as `openai`, `doubao-tts`, `openai-whisper`, `mineru`, `alidocmind`, `seedream`, `seedance`, or `zhihu`. Unknown IDs cannot register custom LLM providers here.
