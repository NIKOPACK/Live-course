---
title: Deployment
description: Deploy LiveCourse to Vercel, Docker, or your own host.
---

LiveCourse is a standard Next.js application. It runs anywhere Next.js runs.

## Vercel

Import the [GitHub repository](https://github.com/NIKOPACK/livecourse) into Vercel and configure an API key for at least one LLM provider in the project settings.
...
The default Compose deployment starts the LiveCourse app and mounts the `livecourse-data` volume. Configure other providers and features as needed; see [Configuration](./configuration.mdx).
...
Inside a Docker container, `localhost` refers to the container itself. If Ollama, Lemonade, VoxCPM, or ComfyUI runs on the host, use `host.docker.internal`, for example `http://host.docker.internal:11434/v1` or `http://host.docker.internal:8188`. Linux Docker usually also requires `extra_hosts: ["host.docker.internal:host-gateway"]` on the `livecourse` service. Because ComfyUI is not currently a server-managed provider, production deployments must also set `ALLOW_LOCAL_NETWORKS=true`, or the SSRF guard will reject the address.
...
The repository's `server-persistence` profile starts LiveCourse and PostgreSQL in two containers. The persistence HTTP API is embedded in LiveCourse; no separate persistence service is required.
...
PostgreSQL data is stored in the `livecourse-postgres` volume. `PERSISTENCE_POSTGRES_PASSWORD` sets the password only when the database directory is first initialized. Changing the environment variable later does not automatically change the existing database user's password.
...
Compose connects LiveCourse to the rendering service through `RENDER_SERVICE_URL`. If the profile is disabled or the rendering service is unavailable, export falls back to downloading the project ZIP for rendering with the local CLI. The rendering service uses an isolated network and requires the `NET_ADMIN` capability at startup. See [`render-service/README.md`](https://github.com/NIKOPACK/livecourse/blob/main/render-service/README.md) for more limitations and standalone deployment instructions.
