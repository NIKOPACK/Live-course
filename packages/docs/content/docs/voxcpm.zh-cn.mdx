...
[VoxCPM2](https://github.com/OpenBMB/VoxCPM) 是 OpenBMB 开源的 TTS 模型，支持声音克隆。LiveCourse 自带适配器，把 VoxCPM 跑在自己机器上即可对接。
...
LiveCourse 支持三种部署形态。三种走的是同一个 LiveCourse 适配器，你只需要在设置里切换对应的后端类型即可。
...
打开 **设置 → 语音合成 → VoxCPM2**，选择后端类型，粘贴 Base URL。下方的 Request URL 预览会显示 LiveCourse 实际请求的地址。
...
LiveCourse 在合成时根据每个 Agent 的人设动态生成 voice prompt。无需任何配置，什么都不做就是这个效果。
