/**
 * @livecourse/render-service — HTTP entrypoint.
 *
...
export function createApp(deps: AppDeps): Hono {
  ...
  app.post('/render', async (c) => {
    ...
    const identity = c.req.header('x-livecourse-client')?.trim() || 'anonymous';
...
async function main(): Promise<void> {
  ...
  // standalone path nothing creates /tmp/livecourse-renders, so without this every
