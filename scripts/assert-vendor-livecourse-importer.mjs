 ...
 *
 * The app loads the parser at runtime from `/vendor/livecourse-importer/index.js` — a
 * gitignored build artifact that `scripts/sync-livecourse-importer.mjs` copies into
...
const entry = path.join(root, 'public/vendor/livecourse-importer/index.js');
...
try {
  ...
  console.error('[assert-vendor] It is loaded at runtime via /vendor/livecourse-importer/index.js');
  console.error('[assert-vendor] and is produced by the postinstall sync step. To fix:');
  console.error(
    '[assert-vendor]   pnpm --filter @livecourse/importer build && pnpm run sync:livecourse-importer',
  );
