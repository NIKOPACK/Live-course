...
describe('resolveLlmText', () => {
  it('keeps visible answer text when present', () => {
    expect(resolveLlmText({ text: '<html></html>', reasoningText: 'scratch' })).toBe(
      '<html></html>',
    );
  });

  it('falls back to reasoning when the answer text is empty', () => {
    expect(
      resolveLlmText({
        text: '  ',
        reasoningText: '<!DOCTYPE html><html><body>ok</body></html>',
      }),
