...
describe('model metadata thinking capabilities', () => {
  ...
  it('assigns fixed thinking only to the compatible Grok reasoning model', () => {
    expect(getCatalogThinkingCapability('grok', 'grok-4.20-0309-reasoning')).toEqual({
      control: 'none',
      requestAdapter: 'none',
      defaultMode: 'enabled',
      toggleable: false,
    ...
    expect(getCatalogThinkingCapability('grok', 'grok-4.20-0309-non-reasoning')).toBeUndefined();
    expect(getCatalogThinkingCapability('grok', 'grok-composer-2.5-fast')).toBeUndefined();
  });
