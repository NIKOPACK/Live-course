...
describe('runClassroomLoad', () => {
  ...
  it('restores a persisted lesson plan from the server fallback instead of leaving a stale or empty plan', () => {
    useStageStore.getState().clearStore();
    useStageStore.getState().setLessonPlan({
      schemaVersion: 1,
      id: 'lesson-plan:stale',
      courseId: 'other',
      stageId: 'other',
      title: 'Stale',
      version: 1,
    ...
    expect(useStageStore.getState().lessonPlan).toEqual(lessonPlan);
    useStageStore.getState().clearStore();
  });
  ...
  it('propagates replay read-only mode and skips roster migration writes', async () => {
    const stage = makeStage('stage-a');
    const { deps, setStage } = makeDeps({
      readOnly: true,
      loadFromStorage: vi.fn().mockImplementation(async (_id, _token, options) => {
