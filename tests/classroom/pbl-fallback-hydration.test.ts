...
import type { KVScope, KVStore, RuntimeSessionInit, RuntimeStore } from '@livecourse/storage';
...
describe('classroom server fallback PBL hydration', () => {
  ...
  it('marks a replay fallback as read-only and disables document persistence', async () => {
    const stage = makeStage('stage-replay');
    const scene = makePBLScene(makeProject());
    const token = claimStageSceneLoadToken();
    const hydrateScenes = vi.fn(async (_stageId: string, scenes: readonly Scene[], options) => {
    ...
    expect(applyStageAndScenes).toHaveBeenCalledWith(
      stage,
      [scene],
