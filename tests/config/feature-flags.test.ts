...
describe('isLiveCourseTTSEnabled', () => {
  const flag = 'NEXT_PUBLIC_ENABLE_LIVECOURSE_TTS';
  ...
  it('defaults to Realtime-only teacher audio', () => {
    delete process.env[flag];
    expect(isLiveCourseTTSEnabled()).toBe(false);
  });

  it('requires an explicit deployment flag before inherited TTS can run', () => {
    process.env[flag] = 'true';
    expect(isLiveCourseTTSEnabled()).toBe(true);
  });
});

const FLAG = 'NEXT_PUBLIC_LIVECOURSE_EDITOR_ENABLED';

describe('isLiveCourseEditorEnabled', () => {
  let original: string | undefined;
  ...
  it('returns false when the env var is unset', () => {
    delete process.env[FLAG];
    expect(isLiveCourseEditorEnabled()).toBe(false);
  });

  it("returns true for 'true'", () => {
    process.env[FLAG] = 'true';
    expect(isLiveCourseEditorEnabled()).toBe(true);
  });
...
describe('isPlaybackRendererEnabled', () => {
  const flag = 'NEXT_PUBLIC_LIVECOURSE_PLAYBACK_RENDERER_ENABLED';
  let original: string | undefined;
...
describe('isPiWebSearchEnabled', () => {
  const flag = 'LIVECOURSE_ENABLE_PI_WEB_SEARCH';
  let original: string | undefined;
...
describe('isVocationalTaskEngineEnabled', () => {
  const flag = 'LIVECOURSE_ENABLE_VOCATIONAL';
  let original: string | undefined;
