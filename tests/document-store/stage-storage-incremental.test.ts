...
const coursePlan: CoursePlan = coursePlanSchema.parse({
  schemaVersion: 1,
...
// A1：教案（AppDocumentOutline.lessonPlan）随大纲快照透传持久化。
describe('outline lessonPlan pass-through', () => {
  ...
  it('carries lessonPlan from the incoming outline into the saved document', async () => {
    await saveStageData('stage-1', { ...data, outline: { ...data.outline!, lessonPlan } }, 0);

    expect(saveDocument).toHaveBeenCalledOnce();
    const saved = saveDocument.mock.calls[0]![0] as { outline: { lessonPlan?: unknown } };
    expect(saved.outline.lessonPlan).toEqual(lessonPlan);
  });

  it('preserves the persisted lessonPlan when a later save omits it', async () => {
    loadDocument.mockReturnValue({
      stage,
      scenes,
      outline: { ...data.outline!, lessonPlan },
      dslVersion: 'current',
    });

    await saveStageData('stage-1', data, 0);
  ...
  it('leaves the outline without lessonPlan when neither side has one', async () => {
    await saveStageData('stage-1', data, 0);

    const saved = saveDocument.mock.calls[0]![0] as { outline: Record<string, unknown> };
    expect('lessonPlan' in saved.outline).toBe(false);
  });
});

describe('coursePlan aggregate save semantics', () => {
  it('preserves an existing coursePlan when the caller omits the field', async () => {
    loadDocument.mockReturnValue({
      stage,
      scenes,
      coursePlan,
    ...
    const saved = saveDocument.mock.calls[0]![0] as { coursePlan?: unknown };
    expect(saved.coursePlan).toEqual(coursePlan);
  });

  it('deletes an existing coursePlan when the caller explicitly passes null', async () => {
    loadDocument.mockReturnValue({
      stage,
      scenes,
      coursePlan,
    ...
    await saveStageData('stage-1', { ...data, coursePlan: null }, 0);

    const saved = saveDocument.mock.calls[0]![0] as { coursePlan?: unknown };
    expect('coursePlan' in saved).toBe(false);
  });

  it('rejects a malformed coursePlan before writing the aggregate', async () => {
    await expect(
      saveStageData('stage-1', { ...data, coursePlan: { courseId: 'wrong' } as never }, 0),
    ).rejects.toThrow();
    expect(saveDocument).not.toHaveBeenCalled();
  });
});
