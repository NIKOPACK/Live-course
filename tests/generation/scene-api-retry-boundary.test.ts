...
describe('scene API retry boundary', () => {
  ...
  it('passes the committed HTML direction to generation without widget fallbacks', async () => {
    const presentation = { mode: 'html', visualStyle: 'Editorial ink and teal diagrams.' };
    ...
    const response = await POST(mockRequest({ presentation }));
