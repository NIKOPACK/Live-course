...
describe('task-engine outline route', () => {
  ...
  test('ignores leftover interactiveMode and uses the standard outline prompt when the server flag is off', async () => {
    vi.resetModules();
    streamLLMMock.mockReset();
    resolveModelFromRequestMock.mockReset();
    ...
    const promptParams = streamLLMMock.mock.calls[0][0] as { system: string; prompt: string };
    expect(promptParams.system).toContain('Scene Outline Generator');
    expect(promptParams.system).not.toContain('Interactive Mode Outline Generator');
