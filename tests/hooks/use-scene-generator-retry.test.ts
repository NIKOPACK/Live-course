...
describe('browser scene generation retry wrappers', () => {
  ...
  it('retries transient scene content HTTP failures before returning success', async () => {
    ...
    for (const call of mockFetch.mock.calls) {
      expect(JSON.parse(call[1].body).presentation).toEqual(presentation);
    }
  });
  ...
  it('does not request TTS while Realtime is the teacher-audio path', async () => {
    delete process.env[LIVECOURSE_TTS_FLAG];
    const { generateAndStoreTTS } = await import('@/lib/hooks/use-scene-generator');

    await expect(generateAndStoreTTS('request-1', 'Hello class')).resolves.toBeNull();
