...
describe('POST /api/chat/pi cue_user', () => {
  ...
  it('does not expose web_search when the server feature flag is disabled', async () => {
    const originalWebSearchFlag = process.env.LIVECOURSE_ENABLE_PI_WEB_SEARCH;
    delete process.env.LIVECOURSE_ENABLE_PI_WEB_SEARCH;
    try {
      mockDirectorWithAgentTurn({ explicitlyCueUser: false });
      const { POST } = await import('@/app/api/chat/pi/route');
      const response = await POST(makeRequest(makeBody()));
      ...
      if (originalWebSearchFlag === undefined) delete process.env.LIVECOURSE_ENABLE_PI_WEB_SEARCH;
      else process.env.LIVECOURSE_ENABLE_PI_WEB_SEARCH = originalWebSearchFlag;
    }
  });
  ...
  it('hands successful web evidence to one child and clears it before later delegations', async () => {
    ...
    process.env.LIVECOURSE_ENABLE_PI_WEB_SEARCH = 'true';
