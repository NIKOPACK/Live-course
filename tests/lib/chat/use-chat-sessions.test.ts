...
describe('presentation-only chat session isolation', () => {
  it('does not restore canonical teaching chats into replay memory', () => {
    const canonical = [makeSession({ id: 'teaching-history', status: 'completed' })];

    expect(getInitialChatSessions(canonical, { presentationOnly: true })).toEqual([]);
    expect(getInitialChatSessions(canonical, { persistSessions: false })).toEqual([]);
    expect(shouldPersistChatSessions({ presentationOnly: true, persistSessions: true })).toBe(
      false,
    );
  });

  it('keeps the canonical persistence callback untouched in replay', () => {
    const persist = vi.fn();
    const replaySession = makeSession({ id: 'replay-session' });

    expect(persistChatSessionsIfEnabled([replaySession], { presentationOnly: true }, persist)).toBe(
      false,
    );
    expect(persist).not.toHaveBeenCalled();

    expect(persistChatSessionsIfEnabled([replaySession], {}, persist)).toBe(true);
    expect(persist).toHaveBeenCalledWith([replaySession]);
  });
});
