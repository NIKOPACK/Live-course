...
const sc = (id: string, actions: Action[], type: Scene['type'] = 'slide'): Scene =>
  ({
    id,
    stageId: 's',
    type,
...
describe('resolvePlaybackCursor', () => {
  ...
  test.each(['quiz', 'interactive', 'pbl'] as const)(
    'a zero-action %s scene also yields one synthetic dwell beat',
    (type) => {
      const scenes = [sc('S0', [], type)];
      expect(resolvePlaybackCursor(scenes, 0, 0)).toMatchObject({
        action: EMPTY_SCENE_DWELL,
        sceneId: 'S0',
      });
