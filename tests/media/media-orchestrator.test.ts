...
import { BrowserAssetStore } from '@livecourse/storage';
...
describe('media orchestrator asset write paths', () => {
  it.each(['interactive', 'quiz'] as const)(
    'reconciles completed generated media into %s HTML without replacing questions or prose',
    (type) => {
      const html = '<html><head></head><body><p>gen_img_1</p><img src="gen_img_1"></body></html>';
      const questions = [{ id: 'q', type: 'short_answer' as const, question: 'What is shown?' }];
      const content = type === 'quiz' ? { type, questions, html } : { type, url: '', html };
      const scene = { id: 'scene-html', stageId, title: 'HTML', type, content, order: 1 } as Scene;
      const stage: Stage = { id: stageId, name: 'HTML course', createdAt: 1, updatedAt: 1 };
      const reconciled = reconcileCompletedMediaForScene(scene, stage, {
        'asset-id': { ...doneTask('asset-id'), placeholderRef: 'gen_img_1' },
      }).scene;
      expect(reconciled.content).toMatchObject({
        html: html.replace('src="gen_img_1"', 'src="asset-id"'),
      });
      if (reconciled.content.type === 'quiz') expect(reconciled.content.questions).toBe(questions);
    },
  );
