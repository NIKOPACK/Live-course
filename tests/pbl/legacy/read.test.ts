...
describe('PBL legacy read support', () => {
  ...
  it('uses a static read-only view for presentation-only playback', () => {
    const content = structuredClone(legacyPBLSceneFixture.content);
    if (content.type !== 'pbl') throw new Error('expected PBL fixture content');
    ...
    expect(markup).toContain('data-pbl-presentation-only="true"');
    expect(markup).toContain('Community Garden Data Project');
    expect(markup).not.toContain('pbl-v2-workspace');
    expect(markup).not.toContain('/api/pbl/v2/');
    expect(markup).not.toMatch(/<(?:button|input|textarea)\b/i);
  });
