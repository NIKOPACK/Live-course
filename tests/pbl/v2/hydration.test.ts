...
import type { KVScope, KVStore } from '@livecourse/storage';
...
describe('PBL runtime hydration', () => {
  ...
  it('read-only hydration never creates a runtime session or backfill snapshot', async () => {
    const store = new MemoryRuntimeStore();
    const kv = new MemoryKVStore();
    const documentProject = transitionProjectUiPhase(makeProject(), 'workspace');
    ...
    expect(hydrated.source).toBe('document');
    expect(hydrated.selfHealed).toBe(false);
    expect(store.sessions).toEqual([]);
    expect(store.records).toEqual([]);
    expect(await kv.keys('runtime.pblDrain.', 'device')).toEqual([]);
  });

  it('read-only hydration does not drain visible project events', async () => {
    const store = new MemoryRuntimeStore();
    const kv = new MemoryKVStore();
    ...
    expect(hydrated.project.uiPhase).toBe('workspace');
    expect(store.sessions).toEqual([]);
    expect(store.records).toEqual([]);
    expect(await kv.keys('runtime.pblDrain.', 'device')).toEqual([]);
  });
