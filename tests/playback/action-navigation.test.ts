...
const LIVECOURSE_TTS_FLAG = 'NEXT_PUBLIC_ENABLE_LIVECOURSE_TTS';
let originalLiveCourseTtsFlag: string | undefined;

beforeEach(() => {
  originalLiveCourseTtsFlag = process.env[LIVECOURSE_TTS_FLAG];
  process.env[LIVECOURSE_TTS_FLAG] = 'true';
});

afterEach(() => {
  if (originalLiveCourseTtsFlag === undefined) delete process.env[LIVECOURSE_TTS_FLAG];
  else process.env[LIVECOURSE_TTS_FLAG] = originalLiveCourseTtsFlag;
});
...
function scene(actions: Action[], type: Scene['type'] = 'slide'): Scene {
  return {
    id: 'scene-1',
    stageId: 'stage-1',
    type,
...
describe('PlaybackEngine action navigation', () => {
  it.each(['quiz', 'interactive', 'pbl'] as const)(
    'completes a zero-action %s scene through the synthetic dwell',
    async (type) => {
      vi.useFakeTimers();
      const { engine: actionEngine } = createActionEngine();
      const { player } = createAudioPlayer(async () => false);
      const onComplete = vi.fn();
      const engine = new PlaybackEngine([scene([], type)], actionEngine, player, { onComplete });
  ...
  it('does not invoke browser TTS while Realtime is the teacher-audio path', async () => {
    delete process.env[LIVECOURSE_TTS_FLAG];
