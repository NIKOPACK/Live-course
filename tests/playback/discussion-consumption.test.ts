...
describe('discussion consumption emits a progress snapshot', () => {
  ...
  it('publishes the fact when classroom replay skips the roundtable protocol', async () => {
    const onProgress = vi.fn();
    const engine = new PlaybackEngine(
      [scene([discussion('disc-replay')])],
      createActionEngine(),
      createAudioPlayer(),
      { onProgress, skipRoundtableDiscussion: true },
    );
    ...
    expect(snapshotsWith(onProgress, 'disc-replay').length).toBeGreaterThan(0);
    expect(engine.getSnapshot().consumedDiscussions).toEqual(['disc-replay']);
  });
