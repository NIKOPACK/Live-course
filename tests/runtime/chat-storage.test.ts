...
import { HttpRuntimeStoreError } from '@livecourse/storage/runtime/http';
...
/**
 * The IndexedDB integration cases below perform hundreds of transactions (up
 * to 300 saves, each touching sessions + records). Under full-suite load they
 * intermittently exceed Vitest's 5 s default while passing in isolation, so
 * each gets a named, explicit bounded per-test timeout — never a global change.
 */
const CHAT_STORAGE_INTEGRATION_TIMEOUT_MS = 20_000;
...
describe('chat RuntimeStore cutover', () => {
  ...
  it(
    'bounds stored and scanned records across a long-lived chat session',
    async () => {
      const store = makeRuntimeStore();
      const legacyStore = new MemoryLegacyChatStore();
    ...
    CHAT_STORAGE_INTEGRATION_TIMEOUT_MS,
  );

  it(
    'preserves the newer cross-tab write when another tab rolls the generation',
    async () => {
      const indexedDB = new IDBFactory();
  ...
  it(
    'keeps the record bound when two tabs roll the same full generation',
    async () => {
      const indexedDB = new IDBFactory();
  ...
  it.each([
    [
      ...
      `@livecourse/storage: cannot append to session "chat:stage-chat:anon%3Achat-test:session-1" with status 'completed' — records may only be appended to an active session`,
    ],
    [
      'post-race reclassification',
      `@livecourse/storage: session "chat:stage-chat:anon%3Achat-test:session-1" is no longer active; its current status is 'completed'`,
    ],
  ...
  it(
    're-resolves the successor when a delayed writer targets a retired generation',
    async () => {
      const indexedDB = new IDBFactory();
