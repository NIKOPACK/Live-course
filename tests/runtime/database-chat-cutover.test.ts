...
import { BrowserRuntimeStore, type KVStore, type RuntimeStore } from '@livecourse/storage';
...
describe('database runtime chat integration', () => {
  ...
  it('loads a divergent destination without marking or deleting its legacy source', async () => {
    ...
    const { getDocumentStore } = await import('@/lib/document-store');
    const { loadStageData } = await import('@/lib/utils/stage-storage');
    const kv = new (await import('@livecourse/storage')).BrowserKVStore();
  ...
  it('upgrades past the abandoned lease schema and adds compound chat restore staging', async () => {
    ...
    const intermediate = new Dexie('LiveCourse-Database', {
      indexedDB: globalThis.indexedDB,
  ...
  it('acquires both the stage-wide and legacy partition Web Lock names', async () => {
    ...
    expect(requested).toEqual([
      'livecourse:chat-storage:all',
      `livecourse:chat-storage:${encodeURIComponent('stage-compatible-lock')}`,
      `livecourse:chat-storage:${encodeURIComponent(`stage-compatible-lock\0${learnerKey}`)}`,
    ]);
  });
  ...
  it('clears document and runtime databases without Web Locks', async () => {
    ...
    localStorage.setItem('livecourse:device:document-migration:stage-clear-document', '{}');
    localStorage.setItem('livecourse:device:editor-current-scene:stage-clear-document', '{}');
    ...
    expect(
      localStorage.getItem('livecourse:device:document-migration:stage-clear-document'),
    ).toBeNull();
    expect(
      localStorage.getItem('livecourse:device:editor-current-scene:stage-clear-document'),
    ).toBeNull();
