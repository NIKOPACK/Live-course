...
describe('model-routes', () => {
  ...
  it('routes the LiveCourse editor agent stage (livecourse-agent)', async () => {
    process.env.MODEL_ROUTES = JSON.stringify({ 'livecourse-agent': 'anthropic:claude-opus-4' });
    const { getStageModel } = await import('@/lib/server/model-routes');
    expect(getStageModel('livecourse-agent')).toBe('anthropic:claude-opus-4');
  });
