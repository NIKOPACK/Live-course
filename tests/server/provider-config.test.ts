...
function clearProviderEnv() {
  ...
  delete process.env.VOLCENGINE_REALTIME_API_KEY;
  delete process.env.IMAGE_OPENAI_ENABLED;
}
...
`;
  ...
  describe('resolveWebSearchApiKey', () => {
    ...
    it('falls back to ZHIHU_API_KEY env var', async () => {
      vi.stubEnv('ZHIHU_API_KEY', 'zhihu-bare-env');
      const { resolveWebSearchApiKey } = await import('@/lib/server/provider-config');
      expect(resolveWebSearchApiKey()).toBe('zhihu-bare-env');
    });

    it('ignores client key and base URL for a server-managed Zhihu provider', async () => {
      vi.stubEnv('ZHIHU_API_KEY', 'zhihu-env-key');
      vi.stubEnv('ZHIHU_BASE_URL', 'https://proxy.example.com/zhihu');
      const { resolveWebSearchApiKey, resolveWebSearchBaseUrl } =
        await import('@/lib/server/provider-config');

      // Managed ⇒ server config is authoritative, client overrides dropped.
      expect(resolveWebSearchApiKey('zhihu', 'zhihu-client-key')).toBe('zhihu-env-key');
      expect(resolveWebSearchBaseUrl('zhihu', 'https://client.example.com')).toBe(
        'https://proxy.example.com/zhihu',
      );
    });

    it('resolves Zhihu API key and base URL from env vars', async () => {
      vi.stubEnv('ZHIHU_API_KEY', 'zhihu-env-key');
      ...
      expect(resolveWebSearchApiKey('zhihu', undefined)).toBe('zhihu-env-key');
...
`;
  ...
  describe('image and video provider metadata', () => {
    ...
    it('does not advertise openai-image when IMAGE_OPENAI_ENABLED=false', async () => {
      vi.stubEnv('OPENAI_API_KEY', 'sk-openai');
      vi.stubEnv('OPENAI_BASE_URL', 'https://proxy.example.com/v1');
  ...
  describe('resolveManagedAliDocMindCredentials (AK/SK)', () => {
    ...
    it('exposes Realtime managed flags from env without leaking keys', async () => {
      vi.stubEnv('OPENAI_API_KEY', 'sk-server');
      ...
      expect(getServerRealtimeProviders()).toEqual({ openai: {}, volc: {} });
    });
