...
describe('disabling the active provider switches selection away', () => {
  ...
  it('web search: disabling the selected provider falls back to the zhihu default', () => {
    const s = useSettingsStore.getState();
    s.setWebSearchProvider('zhihu');
    expect(useSettingsStore.getState().webSearchProviderId).toBe('zhihu');

    // Zhihu is the only web search provider: disabling it while selected
    // settles back on the (single) default rather than a removed provider.
