...
describe('listSelectableModels', () => {
  it('returns an empty list when no provider is usable', () => {
    expect(
      listSelectableModels({
        openai: { apiKey: '', models: [{ id: 'gpt-4o', name: 'GPT-4o' }] },
      }),
    ).toEqual([]);
  });

  it('lists models from usable providers only', () => {
    expect(
      listSelectableModels({
        openai: {
          isServerConfigured: true,
        ...
        anthropic: {
          apiKey: '',
          name: 'Anthropic',
          models: [{ id: 'claude-sonnet-4', name: 'Claude' }],
        },
      }),
    ).toEqual([
      {
