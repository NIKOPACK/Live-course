...
describe('generationComplete', () => {
  ...
  it('does not self-heal generation completion during a read-only replay load', async () => {
    loadStageDataMock.mockResolvedValue({
      stage: makeStage(),
      scenes: [makeSlideScene('a', 1), makeSlideScene('b', 2)],
      currentSceneId: 'a',
    ...
    await useStageStore.getState().loadFromStorage('stage-1', undefined, { readOnly: true });

    expect(useStageStore.getState().generationComplete).toBe(true);
    expect(stageOutlinesPut).not.toHaveBeenCalled();
    expect(hydratePBLScenesFromRuntimeMock).toHaveBeenCalledWith('stage-1', expect.any(Array), {
      readOnly: true,
    });
  });
  ...
  it('heals a missing in-memory cursor when skipping hydration for a live stage', async () => {
    const liveScene = makeSlideScene('live-a', 1);
    const diskScene = makeSlideScene('disk-a', 1);
    loadStageDataMock.mockResolvedValue({
      stage: makeStage(),
      scenes: [diskScene],
    ...
    expect(useStageStore.getState().scenes).toEqual([liveScene]);
    expect(useStageStore.getState().currentSceneId).toBe('live-a');
  });
