...
describe('incremental stage flush', () => {
  ...
  it('selects the first generated scene when the cursor is empty', async () => {
    useStageStore.setState({ scenes: [], currentSceneId: null });
    useStageStore.getState().addScene(scene('scene-1'));
    expect(useStageStore.getState().currentSceneId).toBe('scene-1');
    useStageStore.getState().addScene(scene('scene-2'));
    expect(useStageStore.getState().currentSceneId).toBe('scene-1');
    await flushStageSave();
